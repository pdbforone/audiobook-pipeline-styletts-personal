...
def main():
    ...
    if len(files) == 0:
        console.print("[red]⚠️  No files in Phase 4 - Phase 5 has nothing to process![/red]")
    elif len(files) == 1:
        file_id = list(files.keys())[0]
        chunks = len(files[file_id].get("chunk_audio_paths", []))
        
        if "Gift" in file_id or "magi" in file_id.lower():
            console.print(f"[bold green]✅ Phase 5 will process ONLY Gift of Magi ({chunks} chunks)[/bold green]")
        elif "Confucius" in file_id:
            console.print(f"[bold yellow]⚠️  Phase 5 will process ONLY Confucius ({chunks} chunks)[/bold yellow]")
            console.print("\nIf you want Gift of Magi, run: .\\switch_to_magi.bat")
        else:
            console.print(f"[bold white]Phase 5 will process: {file_id} ({chunks} chunks)[/bold white]")
    else:
        console.print(f"[bold red]⚠️  WARNING: Phase 5 will process ALL {len(files)} books![/bold red]")
        
        total_chunks = sum(len(f.get("chunk_audio_paths", [])) for f in files.values())
