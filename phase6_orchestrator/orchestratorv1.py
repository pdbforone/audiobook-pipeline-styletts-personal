...
def load_pipeline_json(json_path: Path) -> Dict:
    ...
    if not json_path.exists():
        logger.info(
            f"Pipeline JSON not found: {json_path} (will create on first run)"
        )
        return {}
    ...
    try:
        ...
        if not isinstance(data, dict):
            logger.warning("Invalid pipeline.json structure, starting fresh")
            return {}
...
def check_phase_status(
    pipeline_data: Dict, phase_num: int, file_id: str
) -> str:
    """
    ...
    for key in files.keys():
        if file_id in key or key in file_id:
            logger.info(
                f"Phase {phase_num}: Using key '{key}' for file_id '{file_id}'"
            )
...
def run_phase_with_retry(
    ...
    for attempt in range(max_retries + 1):
        if attempt > 0:
            logger.info(
                f"Retry attempt {attempt}/{max_retries} for Phase {phase_num}"
            )
...
def run_phase(
    phase_num: int, file_path: Path, file_id: str, pipeline_json: Path
) -> bool:
    """
    Run a single phase.
    ...
    return run_phase_standard(
        phase_num, phase_dir, file_path, file_id, pipeline_json
    )
...
def run_phase_standard(
    ...
    if phase_num == 3:
        ...
        script_names = {1: "validation.py", 2: "extraction.py", 5: "main.py"}
    ...
    try:
        ...
        if result.returncode != 0:
            logger.error(
                f"Phase {phase_num} FAILED (exit {result.returncode}) in {duration:.1f}s"
            )
...
def run_phase4_with_conda(
    phase_dir: Path, file_id: str, pipeline_json: Path
) -> bool:
    """
    ...
    if RICH_AVAILABLE and console:
        with Progress(
            ...
            task = progress.add_task(
                f"[cyan]Synthesizing {len(chunks)} chunks...",
                total=len(chunks),
            )
    ...
    if failed_chunks:
        logger.warning(
            f"Partial success: {success_count}/{len(chunks)} chunks completed"
        )
...
Examples:
  ...
  python orchestrator.py input/book.pdf --phases 3 4 5
    ...
    parser.add_argument(
        "file", type=Path, help="Input file path (PDF or ebook)"
    )
    parser.add_argument(
        "--pipeline-json",
        type=Path,
...
"""
    ...
    # Load pipeline.json
    pipeline_data = (
        load_pipeline_json(pipeline_json) if not args.no_resume else {}
    )
    ...
    for phase_num in args.phases:
        ...
        if not args.no_resume:
            ...
            if status == "success":
                print_status(
                    f"[green]✓ Skipping {phase_name} (already completed)[/green]"
                )
                completed_phases.append(phase_num)
                continue
            elif status in ["failed", "partial"]:
                print_status(
                    f"[yellow]⟳ Retrying {phase_name} (previous status: {status})[/yellow]"
                )
