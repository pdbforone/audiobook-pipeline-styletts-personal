...
try:
    ...
    print("⚠️  Warning: librosa not available. Audio duration checks will be skipped.")
...
def test_phase2_to_phase3_coverage(
    ...
    """
    print(f"\n{'='*70}")
    print(f"TEST 1: Phase 2 → Phase 3 Text Coverage")
    print(f"{'='*70}")
    ...
    try:
        ...
        extracted_text_path_str = phase2_data.get('extracted_text_path') or phase2_data.get('output_file')
        ...
        if not extracted_text_path.is_absolute():
            extracted_text_path = (pipeline_json_path.parent / extracted_text_path).resolve()
    ...
    try:
        ...
        if isinstance(chunk_paths_data, list):
            ...
            for chunk_id in sorted(chunk_paths_data.keys(), 
                                   key=lambda x: int(x.split('_')[-1]) if '_' in x else int(x)):
                chunk_info = chunk_paths_data[chunk_id]
                chunk_path_str = chunk_info.get('path') if isinstance(chunk_info, dict) else chunk_info
                chunk_path = Path(chunk_path_str)
                # Resolve relative paths
                if not chunk_path.is_absolute():
                    chunk_path = (pipeline_json_path.parent / chunk_path).resolve()
    ...
    if exact_match:
        print(f"  ✅ EXACT MATCH - All text preserved!")
        success = True
    elif similarity > 0.99:
        print(f"  ⚠️  NEAR MATCH - {similarity*100:.2f}% similar (may be whitespace differences)")
        success = True
    else:
...
def test_phase3_to_phase4_coverage(
    ...
    """
    print(f"\n{'='*70}")
    print(f"TEST 2: Phase 3 → Phase 4 Audio Coverage")
    print(f"{'='*70}")
    ...
    if num_text_chunks != len(audio_paths):
        print(f"  ❌ MISMATCH: {abs(num_text_chunks - len(audio_paths))} chunks difference!")
        count_match = False
    else:
        print(f"  ✅ MATCH: All chunks have audio")
        count_match = True
    ...
    if num_samples > 0 and LIBROSA_AVAILABLE:
        ...
        for idx in sample_indices:
            ...
            try:
                ...
                if duration < 0.5:
                    empty_audio.append((idx, str(audio_path), f"too short: {duration:.2f}s"))
                elif rms < 0.001:
                    empty_audio.append((idx, str(audio_path), f"silent: RMS={rms:.6f}"))
        ...
        if empty_audio:
            ...
            print(f"  ✅ All sampled audio files have valid content")
...
def run_coverage_tests(
    ...
    if file_id is None:
        ...
        try:
            ...
            print(f"❌ Error: No files found in pipeline.json")
            return False
    ...
    print(f"SUMMARY")
    print(f"{'='*70}")
    print(f"Test 1 (Phase 2→3 Text Coverage): {'✅ PASS' if test1_success else '❌ FAIL'}")
    print(f"Test 2 (Phase 3→4 Audio Coverage): {'✅ PASS' if test2_success else '❌ FAIL'}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file-id",
        help="File ID to test (default: use first file)"
    )
    parser.add_argument(
        "--show-diff",
        action="store_true",
