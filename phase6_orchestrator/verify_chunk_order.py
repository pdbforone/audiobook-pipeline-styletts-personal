...
def main():
    ...
    console.print("[yellow]Enhanced Files Check[/yellow]")
    ...
    if is_correct:
        console.print(
            "  ✅ [bold green]Chunks are in CORRECT ORDER![/bold green]"
        )
    else:
    ...
    if audiobook_path.exists():
        ...
        if expected_size_range[0] <= size_mb <= expected_size_range[1]:
            console.print("  ✅ Size looks reasonable")
        else:
            console.print(
                f"  ⚠️  Size outside expected range ({expected_size_range[0]}-{expected_size_range[1]} MB)"
            )
    else:
    ...
    if log_path.exists():
        ...
        sort_msgs = [
            l
            for l in last_lines
            if "Creating final audiobook" in l or "chunks in order" in l
        ]
        if sort_msgs:
    ...
    if is_correct and audiobook_path.exists():
        ...
        console.print("\n🎧 Listen to verify:")
        console.print(
            "   Story should start: 'ONE DOLLAR AND EIGHTY-SEVEN CENTS...'"
        )
