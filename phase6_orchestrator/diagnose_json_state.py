...
def main():
    ...
    if audio_chunks_dir.exists():
        ...
        console.print("📁 Phase 4 audio_chunks/")
    ...
    if processed_dir.exists():
        ...
        console.print(
            f"  • audiobook.mp3: {'✅ EXISTS' if audiobook.exists() else '❌ MISSING'}"
        )
    ...
    if len(files) > 1:
        console.print(
            "[yellow]⚠️  Multiple books in pipeline_magi.json[/yellow]"
        )
        console.print("\nYou should:")
        console.print("1. Run check_and_fix_json.py to clean up the JSON")
        console.print(
            "2. Choose which book to keep (Confucius or Gift of Magi)"
        )
