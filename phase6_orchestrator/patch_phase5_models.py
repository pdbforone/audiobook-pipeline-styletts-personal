...
def main():
    ...
    if "ge=5" in content and "snr_threshold" in content:
        ...
        console.print(
            "[green]✓[/green] Patch 1: Allow snr_threshold >= 0.0 (was >= 5.0)"
        )
        patches_applied += 1
    ...
    if "ge=0.1" in content and "noise_reduction_factor" in content:
        ...
        console.print(
            "[green]✓[/green] Patch 2: Allow noise_reduction_factor >= 0.0 (was >= 0.1)"
        )
        patches_applied += 1

    if patches_applied == 0:
        console.print(
            "[yellow]⚠[/yellow]  No validators found to patch (may already be patched)"
        )
