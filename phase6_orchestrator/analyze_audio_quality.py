...
test_audio = Path("../phase4_tts/audio_chunks/chunk_0.wav")  # From test_simple_text.py
real_audio_1 = Path("../phase4_tts/audio_chunks/chunk_1.wav")  # From orchestrator
...
def analyze_audio(audio_path, label):
    ...
    try:
        ...
        print(f"  Spectral centroid range: {spectral_centroids.min():.1f} - {spectral_centroids.max():.1f} Hz")
        ...
        segment_energies = [librosa.feature.rms(y=seg)[0].mean() for seg in segments]
        ...
        if energy_ratio < 0.5:
            print(f"  ⚠️  POSSIBLY REVERSED (energy increases toward end)")
        elif energy_ratio > 2.0:
            print(f"  ⚠️  UNUSUAL ENERGY PATTERN (too much front-loading)")
        else:
            print(f"  ✅ Normal energy distribution")
        ...
        peaks, _ = find_peaks(autocorr[sr//10:], height=autocorr.max()*0.3)
        ...
        if len(peaks) > 0:
            ...
            if len(peaks) > 5:
                print(f"  ⚠️  EXCESSIVE REPETITION - possible TTS loop")
        else:
...
real1_metrics = analyze_audio(real_audio_1, "ORCHESTRATOR AUDIO - CHUNK 1 (GARBAGE)")
real10_metrics = analyze_audio(real_audio_10, "ORCHESTRATOR AUDIO - CHUNK 10 (GARBAGE?)")
...
if test_metrics and real1_metrics:
    ...
    print(f"  Difference: {abs(test_metrics['duration'] - real1_metrics['duration']):.2f}s")
    
    print(f"\nRMS Energy:")
    print(f"  Test: {test_metrics['rms']:.6f}")
    print(f"  Chunk 1: {real1_metrics['rms']:.6f}")
    print(f"  Ratio: {real1_metrics['rms'] / (test_metrics['rms'] + 1e-10):.2f}x")
    ...
    print(f"  Difference: {abs(test_metrics['spectral_centroid'] - real1_metrics['spectral_centroid']):.1f} Hz")
