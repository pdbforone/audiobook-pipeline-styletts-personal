...
if phase2_files:
    ...
    extracted_text_path = phase2_data.get(
        "extracted_text_path"
    ...
    if extracted_text_path and not Path(extracted_text_path).is_absolute():
        ...
        print(
            f"  File exists: {Path(extracted_text_path).exists() if extracted_text_path else 'N/A'}"
        )
        ...
        if chunk_paths_data:
            if isinstance(chunk_paths_data, list):
                ...
                if not Path(first_chunk_path).is_absolute():
                    ...
                    print(
                        f"    File exists: {Path(first_chunk_path).exists()}"
                    )
                ...
                if isinstance(first_chunk_data, dict):
                    ...
                    print(
                        f"    File exists: {Path(chunk_path).exists() if chunk_path else 'N/A'}"
                    )
                else:
                    print(f"    Path: {first_chunk_data}")
                    print(
                        f"    File exists: {Path(first_chunk_data).exists()}"
                    )
    ...
    if file_id in pipeline.get("phase4", {}).get("files", {}):
        ...
        print(
            f"  Phase 4 status: {pipeline.get('phase4', {}).get('status', 'unknown')}"
        )
