...
if len(complete) > 0:
    print(
        f"❌ resume_on_failure=true is skipping {len(complete)} already-complete chunks"
    )
    print(
        f"   Only {len(failed)} + {len(pending)} = {len(failed) + len(pending)} chunks being retried"
    )
else:
    print("✓ No complete chunks found, all should be reprocessed")
...
if phase4_total > phase5_attempted:
    ...
    print(
        "   Likely cause: Phase 5 couldn't find audio files or hit an early error"
    )
elif phase4_total < phase5_attempted:
    print(
        f"\n⚠️  Phase 5 has MORE chunks ({phase5_attempted}) than Phase 4 ({phase4_total})"
    )
    print("   This suggests duplicate processing or stale data")
else:
