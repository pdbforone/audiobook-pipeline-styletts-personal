...
def process_direct_mode(
    audio_dir: Path, output_dir: Path, config: EnhancementConfig, temp_dir: str
    ...
    with Progress(
        ...
        task = progress.add_task(
            f"[cyan]Enhancing {len(chunks)} chunks...", total=len(chunks)
        )
        ...
        with ThreadPoolExecutor(max_workers=config.max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                if (
                    ...
                    enhanced_path = (
                        output_dir / f"enhanced_{metadata.chunk_id:04d}.wav"
                    )
                    sf.write(
                        enhanced_path,
                        enhanced_audio,
...
def main():
    ...
    console.print(
        "[bold cyan]Phase 5 Direct Mode - Bypassing Pipeline[/bold cyan]"
    )
    ...
    if not config_path.exists():
        console.print(
            f"[red]Error: config.yaml not found at {config_path}[/red]"
        )
        return 1
    ...
    console.print("[green]✓[/green] Resume disabled (processing ALL chunks)")
    console.print(
        "[green]✓[/green] Quality validation disabled (no chunks skipped)\n"
    )
    ...
    temp_dir = tempfile.mkdtemp(
        prefix="phase5_direct_", dir=phase5_dir / "temp"
    )
    ...
    try:
        ...
        processed_metadata, enhanced_chunks = process_direct_mode(
            phase4_audio_dir, processed_dir, config, temp_dir
        )
        ...
        console.print(
            f"[green]Processed:[/green] {successful}/{len(processed_metadata)} chunks"
        )
        ...
        console.print(
            f"[cyan]Final audiobook:[/cyan] {output_dir / 'audiobook.mp3'}\n"
        )
