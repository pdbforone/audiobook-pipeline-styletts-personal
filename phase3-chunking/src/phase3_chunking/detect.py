...
def detect_genre(text: str, metadata: Optional[Dict] = None) -> Tuple[str, float, Dict[str, float]]:
    """
    ...
    meta_text = f"{metadata.get('title', '')} {metadata.get('author', '')}".lower()
    
    # Philosophy indicators
    if any(term in meta_text for term in ['philosophy', 'meditations', 'ethics', 'metaphysics', 'epistemology']):
        scores['philosophy'] += 0.3
        logger.debug("Metadata suggests philosophy")
    
    # Fiction indicators
    if any(term in meta_text for term in ['novel', 'fiction', 'story', 'tales', 'narrative']):
        scores['fiction'] += 0.3
        logger.debug("Metadata suggests fiction")
    
    # Academic indicators
    if any(term in meta_text for term in ['introduction', 'textbook', 'studies', 'research', 'theory']):
        scores['academic'] += 0.3
        logger.debug("Metadata suggests academic")
    
    # Memoir indicators
    if any(term in meta_text for term in ['memoir', 'autobiography', 'journal', 'diary', 'life of']):
        scores['memoir'] += 0.3
        logger.debug("Metadata suggests memoir")
    
    # Technical indicators
    if any(term in meta_text for term in ['manual', 'guide', 'handbook', 'tutorial', 'programming']):
    ...
    first_person_count = len(re.findall(r'\b(I|my|me|mine)\b', sample, re.IGNORECASE))
    first_person_ratio = first_person_count / len(sample.split()) if sample else 0
    
    dialogue_tags = len(re.findall(r'\b(said|replied|asked|answered|exclaimed|whispered)\b', sample, re.IGNORECASE))
    dialogue_ratio = dialogue_tags / len(sentences) if sentences else 0
    
    citation_count = len(re.findall(r'\([A-Z][a-z]+,?\s+\d{4}\)', sample))  # (Author, YEAR)
    citation_ratio = citation_count / len(sentences) if sentences else 0
    
    code_markers = len(re.findall(r'```|def\s+\w+\(|function\s+\w+|class\s+\w+', sample))
    ...
    if avg_sentence_len > 25:
        ...
        logger.debug(f"Long sentences ({avg_sentence_len:.1f} avg) suggest philosophy/academic")
    ...
    # Lists/enumerations
    list_markers = len(re.findall(r'^\s*\d+\.|^\s*[a-z]\)', sample, re.MULTILINE))
    if list_markers > 3:
        scores['academic'] += 0.1
    ...
    if first_person_ratio > 0.03:
        scores['memoir'] += 0.3
        logger.debug(f"First-person narrative ({first_person_ratio:.3f}) suggests memoir")
    
    # Check for anecdotal style
    anecdote_markers = len(re.findall(
    ...
    if code_markers > 0 or formula_markers > 2:
        ...
        logger.debug(f"Code/formulas detected (code: {code_markers}, formulas: {formula_markers})")
    ...
    if confidence < 0.3:
        logger.warning(f"Low genre confidence ({confidence:.2f}), using 'auto' profile")
...
def validate_genre(genre: str) -> bool:
    ...
    """
    valid_genres = {'philosophy', 'fiction', 'academic', 'memoir', 'technical', 'auto'}
