...
def get_nlp():
    ...
    if _nlp is None:
        ...
        logger.info(
            f"spaCy model loaded with max_length: {_nlp.max_length:,} chars"
        )
    return _nlp
...
def split_at_clauses(text: str, max_chars: int) -> List[str]:
    ...
    parts = re.split(
        r",\s+(and|but|or|nor|for|yet|so)\s+", text, flags=re.IGNORECASE
    )
...
def try_complete_chunk_aggressive(
    ...
    for i, sent in enumerate(remaining_sentences):
        ...
        if test_len <= hard_limit and test_duration <= MAX_DURATION_SECONDS:
            ...
            test_len <= emergency_limit
            and test_duration <= MAX_DURATION_SECONDS
        ):
            # Emergency extension - only if it completes the chunk
            is_complete, _ = is_complete_chunk(test_chunk)
            if is_complete:
                logger.info(
                    f"Emergency extension to {test_len} chars to complete chunk"
                )
...
def chunk_with_flexible_limits(
    ...
    while i < len(sentences):
        ...
        if sent_len > hard_limit or sent_duration > max_duration:
            logger.warning(
                f"Sentence exceeds HARD_LIMIT ({sent_len} chars), splitting at semicolons/clauses"
            )
            ...
            if current_chunk:
                ...
                if not is_complete:
                    ...
                    chunk_text, remaining, was_completed = (
                        try_complete_chunk_aggressive(
                            chunk_text,
                            sentences[i:],
                            hard_limit,
                            emergency_limit,
                ...
                if max(len(c) for c in sub_chunks) > hard_limit:
                    ...
                    for sub in sub_chunks:
                        if len(sub) > hard_limit:
                            final_chunks.extend(
                                split_at_clauses(sub, hard_limit)
                            )
                        else:
            ...
            if current_chunk:
                ...
                if not is_complete:
                    ...
                    if not was_completed:
                        ...
                        if chunks:
                            logger.info(
                                f"Merging incomplete chunk backwards: {reason}"
                            )
                            prev_chunk = chunks[-1]
                            merged = prev_chunk + " " + chunk_text

                            if (
                                len(merged) <= emergency_limit
                                and predict_duration(merged) <= max_duration
                            ):
                                # Merge successful
                                chunks[-1] = merged
                            else:
                                ...
                                logger.info(
                                    f"Merged chunk too large ({len(merged)} chars), splitting at semicolons"
                                )
                                sub_chunks = split_at_semicolons(
                                    merged, hard_limit
                                )
                                if (
                                    max(len(c) for c in sub_chunks)
                                    > hard_limit
                                ):
                                    # Try clauses
                                    final_chunks = []
...
def form_semantic_chunks(
    ...
    if incomplete_chunks:
        logger.error(
            f"❌ CRITICAL: {len(incomplete_chunks)} chunks are INCOMPLETE after processing!"
        )
    ...
    if len(chunks) > len(valid_chunks):
        logger.warning(
            f"Dropped {len(chunks) - len(valid_chunks)} invalid/short chunks"
        )
    ...
    if oversized:
        logger.error(
            f"❌ CRITICAL: {len(oversized)} chunks exceed {max_duration}s!"
        )
