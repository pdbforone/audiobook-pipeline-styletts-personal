...
def find_voice_registry() -> Path:
    ...
    phase3_configs = Path(__file__).resolve().parent / "configs" / "voices.json"
...
def load_voice_registry() -> Dict:
    ...
    try:
        ...
        logger.debug(f"Available voices: {list(registry.get('voices', {}).keys())}")
    ...
    if not selected_voice:
        try:
            ...
            if matching_voices:
                ...
                selection_reason = f"Profile match ({profile_name} → {selected_voice})"
...
def get_voice_params(voice_id: str) -> Dict:
    ...
    try:
        ...
        if voice_id not in voices:
            logger.warning(f"Voice '{voice_id}' not found, using default params")
            return {}
...
def print_voice_info(voice_id: Optional[str] = None) -> None:
    ...
    try:
        ...
        if voice_id:
            ...
            print(f"Profiles: {', '.join(voice_data.get('preferred_profiles', []))}")
            ...
            for vid, vdata in voices.items():
                ...
                print(f"   Profiles: {', '.join(vdata.get('preferred_profiles', []))}")
                print()
...
Examples:
  ...
  python voice_selection.py --info jim_locke
    ...
    parser.add_argument(
        "--profile",
        help="Filter voices by genre profile"
    )
    parser.add_argument(
        "--info",
        help="Show details for specific voice"
    )
    parser.add_argument(
        "--set-global",
        metavar="VOICE_ID",
        help="Set global voice override"
    )
    parser.add_argument(
        "--set-file",
        nargs=2,
