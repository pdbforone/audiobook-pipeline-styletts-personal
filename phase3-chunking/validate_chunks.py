...
def is_complete_chunk(text: str) -> Tuple[bool, str]:
    ...
    incomplete_patterns = [
        ...
        # Articles
        (r"\b(the|a|an)\s*$", "article"),
        # Relative pronouns
        (r"\b(which|that|who|whom|whose|where|when)\s*$", "relative pronoun"),
        ...
        (r"\b(and|but|or|yet|so|nor|for)\s*$", "coordinating conjunction"),
        # Auxiliary verbs
        (
    ...
    for pattern, pattern_name in incomplete_patterns:
        ...
        if match:
            return (
                False,
                f"ends with {pattern_name} ('{match.group().strip()}')",
            )
...
def validate_chunks_in_directory(chunks_dir: str) -> None:
    ...
    if incomplete_chunks:
        ...
        for i, chunk_info in enumerate(incomplete_chunks, 1):
            ...
            print(
                f"  Size: {chunk_info['chars']} chars, {chunk_info['words']} words"
            )
    ...
    print(f"{'-'*80}")
    print(
        f"✅ Complete chunks: {len(complete_chunks)} ({len(complete_chunks)/len(chunk_files)*100:.1f}%)"
    )
    print(
        f"❌ Incomplete chunks: {len(incomplete_chunks)} ({len(incomplete_chunks)/len(chunk_files)*100:.1f}%)"
    )

    if complete_chunks:
        avg_chars = sum(c["chars"] for c in complete_chunks) / len(
            complete_chunks
        )
        avg_words = sum(c["words"] for c in complete_chunks) / len(
            complete_chunks
        )
