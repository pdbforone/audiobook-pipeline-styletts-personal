...
def create_cover_art(title, author, output_path, width=1920, height=1080):
    ...
    for dx in [-2, 0, 2]:
        for dy in [-2, 0, 2]:
            draw.text((title_x + dx, title_y + dy), title, font=title_font, fill=(0, 0, 0))
    ...
    for dx in [-2, 0, 2]:
        for dy in [-2, 0, 2]:
            draw.text((author_x + dx, author_y + dy), author_text, font=author_font, fill=(0, 0, 0))
...
def create_video(audio_path, cover_path, subtitle_path, output_path):
    ...
    if cover_path and cover_path.exists():
        ...
        cmd = [
            ...
            '-i', str(audio_path),
            '-vf', f"subtitles={subtitle_escaped}:{subtitle_style}",
...
#Audiobook #ClassicLiterature #ELL #PublicDomain #EnglishLearning""",
        'tags': ['audiobook', 'full audiobook', 'audiobook with subtitles', 
                'classic literature', 'public domain', 'ELL', 'English language learning',
...
def main():
    parser = argparse.ArgumentParser(description='Phase 6.5: Simplified Video Assembly')
    parser.add_argument('--audio', type=Path, required=True, help='Audio file path')
    parser.add_argument('--subtitles', type=Path, required=True, help='SRT subtitle file')
    parser.add_argument('--title', type=str, required=True, help='Book title')
    parser.add_argument('--author', type=str, required=True, help='Book author')
    parser.add_argument('--output-dir', type=Path, default=Path('output'), help='Output directory')
    parser.add_argument('--no-cover', action='store_true', help='Skip cover art generation')
