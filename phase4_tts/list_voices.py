...
def list_built_in_voices(engine: str = None, profile: str = None):
    ...
    for engine_name, voices in built_in.items():
        ...
        for voice_name, voice_data in voices.items():
            if profile and profile.lower() not in [
                p.lower() for p in voice_data.get("preferred_profiles", [])
            ]:
                continue
            ...
            print(f"    {desc}")
            print(
                f"    Gender: {gender.capitalize()} | Accent: {accent} | Quality: {quality}"
            )
...
def list_custom_voices(profile: str = None):
    ...
    for voice_name, voice_data in references.items():
        ...
        source = (
            "Local file"
            if has_local
            else ("URL download" if has_url else "Unknown")
        )

        print(f"  • {voice_name}")
        print(f"    {desc}")
        print(
            f"    Narrator: {narrator} | Source: {source} | Quality: {quality}"
        )
        print(f"    Best for: {profiles}")
        print()
...
def main():
    ...
    parser.add_argument(
        "--engine", choices=["xtts", "kokoro"], help="Filter by engine"
    )
    parser.add_argument(
        "--profile", help="Filter by profile (e.g., philosophy, fiction)"
    )
    parser.add_argument(
        "--built-in-only",
        action="store_true",
        help="Show only built-in voices",
    )
    parser.add_argument(
        "--custom-only",
        action="store_true",
        help="Show only custom voice clones",
    )
    ...
    print(
        '  python engine_runner.py --engine xtts --voice "Claribel Dervla" --file_id MyBook ...'
    )
    print(
        "  python engine_runner.py --engine kokoro --voice af_sarah --file_id MyBook ..."
    )
    print("\nTo use a custom voice clone:")
    print(
        "  python engine_runner.py --engine xtts --voice bob_neufeld --file_id MyBook ..."
    )
    print("\nVoice cloning (XTTS only):")
    print(
        "  Add your reference audio to voice_references.json with 'local_path'"
    )
