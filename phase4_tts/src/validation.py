...
class ValidationResult:
    ...
    is_valid: bool
    tier: int  # 1 or 2
    reason: str  # e.g., "duration_mismatch", "error_phrase_detected", "high_wer"
...
def has_silence_gap(audio_path: str, threshold_sec: float = 2.0, silence_threshold_db: float = -40.0) -> Tuple[bool, float]:
    """
...
def is_too_quiet(audio_path: str, threshold_db: float = -40.0) -> Tuple[bool, float]:
    """
...
def has_error_phrase_pattern(audio_path: str, error_phrases: list) -> Tuple[bool, Optional[str]]:
    """
...
def tier1_validate(
    ...
    suspected, pattern_reason = has_error_phrase_pattern(audio_path, config.error_phrases)
...
def tier2_validate(
    ...
    try:
        ...
        # Transcribe audio
        logger.info(f"Transcribing audio with Whisper {config.whisper_model}...")
...
def validate_audio_chunk(
    ...
    if should_run_tier2_validation(chunk_idx, total_chunks, config):
        logger.info(f"Running Tier 2 validation (chunk {chunk_idx+1}/{total_chunks})...")
