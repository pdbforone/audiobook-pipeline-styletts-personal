...
def strip_and_stitch(chunks_dir: Path, output_mp3: Path, strip_seconds: float = 3.0):
    """Strip first N seconds from each chunk and re-stitch."""
    
    wav_files = sorted(
        chunks_dir.glob("*.wav"),
        key=lambda f: extract_chunk_number(f.name)
    )
    ...
    logger.info(f"Processing {len(wav_files)} chunks (stripping first {strip_seconds}s each)...")
    ...
    for i, wav_path in enumerate(wav_files, 1):
        ...
        if len(audio) > strip_ms:
            ...
            logger.warning(f"Chunk {i} shorter than {strip_seconds}s, skipping trim")
            trimmed = audio
    ...
    result.export(
        ...
        format="mp3",
        bitrate="192k",
        tags={"title": "Meditations (Start-Stripped)", "artist": "Marcus Aurelius"}
    )
