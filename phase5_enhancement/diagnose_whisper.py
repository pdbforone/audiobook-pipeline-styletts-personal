...
def diagnose_transcription(input_mp3: Path, sample_timestamp: float = 203.7):
    ...
    segments, info = model.transcribe(
        str(sample_path), beam_size=5, word_timestamps=True, language="en"
    )
    ...
    for segment in segments:
        ...
        if hasattr(segment, "words") and segment.words:
            ...
            for word in segment.words:
                ...
                logger.info(
                    f"    [{word_count}] {word.start:.2f}s-{word.end:.2f}s: {word_str}"
                )
    ...
    for i in range(len(words_stripped) - len(target_words) + 1):
        window = [
            w.lower().strip(".,!?;:")
            for w in words_stripped[i : i + len(target_words)]
        ]
