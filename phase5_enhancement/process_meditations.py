...
from phase5_enhancement.phrase_cleaner import (
    PhraseCleaner,
    PhraseCleanerConfig,
)
...
def main():
    ...
    if config.enable_phrase_cleanup:
        ...
        logger.info(
            f"[OK] Phrase cleaner ready (model: {config.cleanup_whisper_model})"
        )
    ...
    sorted_files = sorted(
        wav_files, key=lambda f: extract_chunk_number(f.name)
    )
    logger.info(
        f"Processing chunks 1-{extract_chunk_number(sorted_files[-1].name)}"
    )
    ...
    for i, wav_path in enumerate(sorted_files, 1):
        try:
            ...
            if phrase_cleaner:
                ...
                if (
                    cleaned_audio is not None
                    and cleanup_meta.get("status") == "cleaned"
                ):
                    audio = cleaned_audio
                    ...
                    if phrases_removed > 0:
                        logger.info(
                            f"  Chunk {chunk_num}: Removed {phrases_removed} phrase(s)"
                        )
                else:
    ...
    logger.info(
        f"Successfully enhanced {len(enhanced_chunks)}/{len(sorted_files)} chunks"
    )
    if phrase_cleaner:
        logger.info(
            f"Phrase cleanup: {phrases_removed_total} phrases removed from {chunks_cleaned} chunks"
        )
    ...
    logger.info(
        f"Phrases removed: {phrases_removed_total} from {chunks_cleaned} chunks"
    )
