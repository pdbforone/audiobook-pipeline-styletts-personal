...
class AudioProcessor:
    ...
    def _noise_gate(
        self, audio: np.ndarray, sr: int, params: Dict
    ) -> np.ndarray:
    ...
    def _eq(self, audio: np.ndarray, sr: int, params: Dict) -> np.ndarray:
        ...
        if "high_pass_hz" in params:
            effects.append(
                HighpassFilter(cutoff_frequency_hz=params["high_pass_hz"])
            )
    ...
    def _limiter(self, audio: np.ndarray, sr: int, params: Dict) -> np.ndarray:
    ...
    def _lufs_normalize(
        ...
        try:
            ...
            logger.warning(
                "pyloudnorm not installed, using peak normalization"
            )
...
class MasteringEngine:
    ...
    def master(
        ...
        metrics = self._calculate_metrics(
            processed, sample_rate, preset_config
        )
    ...
    def _calculate_metrics(
        self, audio: np.ndarray, sample_rate: int, preset_config: Dict
    ) -> Dict[str, float]:
        ...
        metrics["peak_db"] = float(
            20 * np.log10(metrics["peak"]) if metrics["peak"] > 0 else -np.inf
        )
        ...
        if percentile_10 > 0:
            metrics["dynamic_range_db"] = float(
                20 * np.log10(percentile_90 / percentile_10)
            )
        else:
        ...
        if "dynamic_range_db" in targets:
            ...
            metrics["dynamic_range_compliance"] = (
                abs(metrics["dynamic_range_db"] - target_dr) < 2.0
            )
