...
class SubtitleGenerator:
    ...
    def initialize(self):
        ...
        logger.info(
            f"Audio duration: {self.audio_duration:.2f}s ({self.audio_duration/60:.1f} minutes)"
        )
    ...
    def transcribe(self) -> List[Dict[str, Any]]:
        ...
        segments_data = []
        checkpoint_path = (
            self.config.output_dir / f"{self.config.file_id}_checkpoint.json"
        )
        ...
        for segment in segments_iter:
            ...
            if (
                ...
                logger.debug(
                    f"Captured {len(segment_data['words'])} words at {segment.start:.2f}s"
                )

            # Handle silence/non-speech
            if (
                segment.no_speech_prob > 0.8
                and (segment.end - segment.start) > 3.0
            ):
            ...
            if self.config.enable_checkpoints:
                ...
                if checkpoint_counter >= self.config.checkpoint_interval:
                    self._save_checkpoint(
                        checkpoint_path, segments_data, segment.end
                    )
                    checkpoint_counter = 0

        duration = time.perf_counter() - start
        logger.info(
            f"Transcription complete: {len(segments_data)} segments in {duration:.2f}s"
        )
    ...
    def _save_checkpoint(
        self, path: Path, segments: List[Dict], last_end: float
    ):
    ...
    def process_segments(self) -> List[Dict[str, Any]]:
        ...
        for seg in self.segments:
            ...
            should_split = (
                combined_length > self.config.max_chars
    ...
    def align_with_aeneas(self, reference_text: Path) -> Optional[List[Dict]]:
        ...
        try:
            ...
            logger.warning(
                "aeneas produced no segments; falling back to Whisper timestamps"
            )
            return None
    ...
    def align_and_validate(
        ...
        coverage = (
            last_timestamp / self.audio_duration
        ...
        if not coverage_ok:
            logger.warning(
                f"Low coverage: {coverage:.2%} (target: {self.config.min_coverage:.0%})"
            )
        ...
        if (
            self.config.reference_text_path
            and self.config.reference_text_path.exists()
        ):
            with open(
                self.config.reference_text_path, "r", encoding="utf-8"
            ) as f:
            ...
            if wer > self.config.max_wer:
                logger.warning(
                    f"High WER: {wer:.2%} (target: <{self.config.max_wer:.0%})"
                )
        else:
            metrics["wer"] = None
    ...
    def save_subtitles(
        self, segments: List[Dict], raw_segments: List[Dict] = None
    ):
        ...
        if self.enable_karaoke:
            ass_path = (
                self.config.output_dir / f"{self.config.file_id}_karaoke.ass"
            )
            ...
            segments_for_karaoke = (
                raw_segments if raw_segments is not None else segments
            )
            karaoke_stats = karaoke_gen.generate_ass(
                segments_for_karaoke, ass_path
            )
    ...
    def save_metrics(self):
        ...
        metrics_path = (
            self.config.output_dir / f"{self.config.file_id}_metrics.json"
        )
    ...
    def generate(self) -> Dict[str, Any]:
        ...
        try:
            ...
            final_segments, metrics = self.align_and_validate(
                processed_segments
            )
            ...
            logger.info(
                f"=== Subtitle generation complete in {total_duration:.2f}s ==="
            )
            ...
            if self.enable_karaoke:
                logger.info("Karaoke mode: ENABLED (ASS file generated)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate subtitles from audiobook"
    )
    ...
    if result["status"] == "success":
        ...
        if result["metrics"].get("karaoke_enabled"):
            ...
            print(
                f"  Avg word duration: {karaoke_stats.get('avg_word_duration', 0):.3f}s"
            )
        print(
            f"  Processing time: {result['metrics']['processing_time']:.1f}s"
        )
    else:
        print(f"\n❌ Failed: {result['error']}")
        exit(1)
