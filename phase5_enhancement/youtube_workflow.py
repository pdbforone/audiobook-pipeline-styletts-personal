...
Examples:
  ...
  python youtube_workflow.py --audio meditations_cleaned.mp3 --skip-clean
    ...
    parser.add_argument(
        "--title", default="The Meditations", help="Book title"
    )
    parser.add_argument(
        "--author", default="Marcus Aurelius", help="Author name"
    )

    # Workflow control
    parser.add_argument(
        "--all", action="store_true", help="Run complete workflow"
    )
    parser.add_argument(
        "--clean-only", action="store_true", help="Only clean audio"
    )
    parser.add_argument(
        "--skip-clean", action="store_true", help="Skip cleaning step"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Dry run for cleaning"
    )

    # Optional overrides
    parser.add_argument(
        "--output-dir", help="Output directory (default: ./youtube_ready)"
    )
    ...
    if not args.skip_clean:
        ...
        cleaned_audio = (
            output_dir / f"{audio_file.stem}_cleaned_{timestamp}.mp3"
        )
    ...
    if args.cover:
        ...
        if not cover_image.exists():
            ...
            create_cover_image(
                cover_image, title=args.title, author=args.author
            )
