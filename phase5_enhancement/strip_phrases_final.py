...
def strip_phrases(input_mp3: Path, output_mp3: Path):
    ...
    segments, _ = model.transcribe(
        str(input_mp3), beam_size=5, word_timestamps=False, language="en"
    )
    ...
    for segment in segments:
        ...
        for phrase in TARGET_PHRASES:
            if phrase in text:
                ...
                logger.info(
                    f"  FOUND at {segment.start:.1f}s: '{segment.text}'"
                )
                break
    ...
    result.export(
        ...
        format="mp3",
        bitrate="192k",
        tags={"title": "Meditations (Cleaned)", "artist": "Marcus Aurelius"},
    )
    ...
    logger.info("=" * 60)
    logger.info(
        f"DONE! Removed {len(matches)} phrases ({removed_mins:.1f} minutes)"
    )
    logger.info(
