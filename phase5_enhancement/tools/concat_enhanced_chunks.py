...
def run_ffmpeg(cmd: list[str], desc: str, logs_dir: Path) -> None:
    ...
    if result.returncode != 0:
        ...
        LOG.error("FFmpeg %s failed (exit %s). See %s", desc, result.returncode, str(log_path))
        LOG.error("FFmpeg stderr (preview): %s", stderr_preview[:3000])
        raise RuntimeError(f"FFmpeg {desc} failed (exit {result.returncode}). See {log_path}\n{stderr_preview[:3000]}")
...
def find_enhanced_wavs(input_dir: Path) -> list[Path]:
    ...
    if any(n > 0 for n in nums):
        files_sorted = sorted(files, key=lambda p: extract_chunk_number_from_filename(str(p)))
    else:
...
def build_and_concat_batches(
    ...
    try:
        ...
        for idx in range(0, len(wavs), batch_size):
            ...
            batch_list.write_text("\n".join(f"file '{p.resolve().as_posix()}'" for p in batch), encoding="utf-8")
        ...
        try:
            ...
            try:
                ...
                LOG.warning("Final ffmpeg convert failed; raw-copied merged file to %s", out_wav)
                return out_wav
            except Exception:
                raise
...
def encode_mp3_from_wav(wav_path: Path, mp3_path: Path, sample_rate: int = 24000, bitrate: str = "128k") -> None:
    logs_dir = Path(__file__).resolve().parents[1] / "logs"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--input-dir", type=str, default=None, help="Directory containing enhanced_*.wav files. If omitted, use --file-id to locate phase4_tts/audio_chunks/<file-id>")
    parser.add_argument("--file-id", type=str, default=None, help="File id folder under phase4_tts/audio_chunks to use when --input-dir is omitted")
    parser.add_argument("--output-wav", type=str, default=None, help="Path to write merged WAV (default: <input-dir>/merged.wav)")
    parser.add_argument("--encode-mp3", action="store_true", help="Also encode MP3 from the merged WAV")
    parser.add_argument("--mp3-bitrate", type=str, default="128k", help="MP3 bitrate to use")
    parser.add_argument("--sample-rate", type=int, default=None, help="Sample rate for final outputs (auto-detect if omitted)")
    parser.add_argument("--crossfade-sec", type=float, default=0.05, help="Crossfade seconds when merging batches")
    parser.add_argument("--batch-size", type=int, default=None, help="Batch size for concat demuxer (auto if omitted)")
    parser.add_argument("--temp-dir", type=str, default=None, help="Optional temp dir to use")
    parser.add_argument("--keep-temp", action="store_true", help="Keep temporary batch files (for debugging)")
    parser.add_argument("--resume", action="store_true", help="If merged output exists, reuse it and skip merging")
    parser.add_argument("--use-pipeline-json", action="store_true", help="Use pipeline.json to obtain ordered chunk paths for the given --file-id")
    parser.add_argument("--pipeline-json", type=str, default=None, help="Path to pipeline.json to use when --use-pipeline-json is set")
    parser.add_argument("--list-only", action="store_true", help="List detected enhanced WAVs and exit (dry run)")
    ...
    if args.input_dir:
        ...
        if not file_id:
            LOG.error("No --input-dir provided and no --file-id/PHASE5_FILE_ID available to locate audio chunks")
            return 2
        ...
        if processed_root.exists() and processed_root.is_dir():
            ...
            for p in processed_root.iterdir():
                ...
                if name.startswith(file_id) or file_id in name or file_norm in name_norm or name_norm.startswith(file_norm):
                    candidates.append(p)
                else:
                    # accept matches where leading digits match and rest is title
                    m_name = re.match(r"^(\d+)", name)
                    m_file = re.match(r"^(\d+)", file_id)
                    if m_name and m_file and m_name.group(1) == m_file.group(1):
                        candidates.append(p)
            if candidates:
        ...
        if input_dir is None:
            ...
            if legacy.exists():
                ...
                LOG.error("Could not locate audio directory for file-id %s (checked %s and %s)", file_id, processed_root, legacy)
                return 2
    ...
    out_wav = Path(args.output_wav) if args.output_wav else input_dir / "merged.wav"
    ...
    if args.use_pipeline_json and args.file_id:
        try:
            pj = Path(args.pipeline_json) if args.pipeline_json else Path("pipeline.json")
            if pj.exists():
                import json
                data = json.loads(pj.read_text(encoding="utf-8"))
                phase4_files = data.get("phase4", {}).get("files", {})
                entry = phase4_files.get(args.file_id) or {}
                chunk_paths = entry.get("chunk_audio_paths") or entry.get("artifacts", {}).get("chunk_audio_paths") or []
                ordered = []
                ...
                if ordered:
                    wavs = ordered
                    LOG.info("Using %d ordered enhanced WAVs from pipeline.json", len(wavs))
        except Exception as e:
    ...
    if sample_rate is None:
        try:
            ...
            LOG.info("Auto-detected sample rate %d from %s", sample_rate, wavs[0])
        except Exception:
            sample_rate = 24000
            LOG.warning("Failed to autodetect sample rate; defaulting to %d", sample_rate)
    ...
    if batch_size is None:
        if total_wavs > 2000:
            ...
            batch_size = 200
        LOG.info("Adaptive batch_size chosen: %d for %d files", batch_size, total_wavs)
    ...
    if args.resume and out_wav.exists():
        try:
            sf.info(str(out_wav))
            LOG.info("Reusing existing merged WAV at %s (resume enabled)", out_wav)
            merged = out_wav
        except Exception:
    ...
    if args.encode_mp3:
        ...
        try:
            encode_mp3_from_wav(merged, mp3_path, sample_rate=sample_rate, bitrate=args.mp3_bitrate)
        except Exception as e:
        ...
        try:
            ...
            filename = (args.file_id + ".mp3") if args.file_id else mp3_path.name
