...
def is_exact_match(
    transcribed_text: str, target_phrase: str, threshold: float = 0.95
) -> bool:
    """
...
def clean_audiobook(input_mp3: Path, output_mp3: Path, dry_run: bool = False):
    ...
    for segment in segments:
        ...
        for phrase in TARGET_PHRASES:
            if is_exact_match(text, phrase, threshold=0.95):
                ...
                logger.info(
                    f"✓ MATCH at {segment.start:.1f}s ({duration:.1f}s)"
                )
    ...
    if false_positives:
        logger.warning(
            f"Found {len(false_positives)} near-matches (NOT removing):"
        )
        for fp in false_positives[:5]:  # Show first 5
            logger.warning(
                f"  {fp['start']:.1f}s: '{fp['text']}' (similarity: {fp['similarity']:.1%})"
            )
    ...
    logger.info(
        f"  Total removal: {total_removed_minutes:.2f} minutes ({total_removed_seconds:.1f}s)"
    )
    logger.info(f"  Original duration: {original_duration_min:.1f} minutes")
    logger.info(
        f"  Expected final duration: {original_duration_min - total_removed_minutes:.1f} minutes"
    )
    logger.info(
        f"  Percentage removed: {(total_removed_minutes / original_duration_min * 100):.2f}%"
    )

    if dry_run:
        logger.info("\n🔍 DRY RUN - No changes made")
        logger.info("Run without --dry-run to create cleaned file")
        return
    ...
    for i, match in enumerate(sorted(matches, key=lambda x: x["start"]), 1):
        ...
        if start_ms > last_end_ms:
            ...
            logger.info(
                f"  Keeping: {last_end_ms/1000:.1f}s to {start_ms/1000:.1f}s"
            )

        logger.info(
            f"  Removing {i}/{len(matches)}: {start_ms/1000:.1f}s to {end_ms/1000:.1f}s"
        )
        last_end_ms = end_ms
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Clean repetitive phrases from audiobook"
    )
    parser.add_argument(
        "--input",
        default="processed/meditations_audiobook.mp3",
        help="Input MP3 file",
    )
    parser.add_argument(
        "--output",
        default="processed/meditations_cleaned_v2.mp3",
