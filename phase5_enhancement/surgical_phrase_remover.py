...
TARGET_PHRASE_WORDS = ["you", "need", "to", "add", "some", "text", "for", "me", "to", "talk"]
...
def find_phrase_in_words(words: List[dict]) -> List[Tuple[int, int]]:
    ...
    while i <= len(words) - phrase_len:
        ...
        window = [w['word'].lower().strip('.,!?;:') for w in words[i:i+phrase_len]]
        
        if window == TARGET_PHRASE_WORDS:
            matches.append((i, i + phrase_len - 1))
            logger.info(f"  Found phrase at word index {i} ({words[i]['start']:.1f}s)")
...
def surgical_remove_phrases(input_mp3: Path, output_mp3: Path):
    ...
    for start_idx, end_idx in matches:
        ...
        logger.info(f"  → Will remove {start_time_ms/1000:.1f}s to {end_time_ms/1000:.1f}s ({duration:.1f}s)")
    ...
    logger.info(f"   Legitimate content PRESERVED ✓")
    logger.info("=" * 70)
