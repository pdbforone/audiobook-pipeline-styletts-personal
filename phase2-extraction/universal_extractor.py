...
class UniversalExtractor:
    ...
    def extract_docx(self) -> str:
        ...
        try:
            ...
            text = '\n\n'.join(paragraphs)
            logger.info(f"✓ Extracted {len(paragraphs)} paragraphs, {len(text):,} chars")
            return text
    ...
    def extract_epub(self) -> str:
        ...
        try:
            ...
            text = '\n\n'.join(chapters)
            logger.info(f"✓ Extracted {len(chapters)} chapters, {len(text):,} chars")
            return text
            
        except ImportError:
            logger.error("ebooklib not installed: pip install ebooklib beautifulsoup4")
            return ""
        except Exception as e:
    ...
    def extract_and_normalize(self) -> Tuple[str, Dict]:
        ...
        if self.file_type == '.pdf':
            ...
            return "", {"status": "failed", "error": f"Unsupported file type: {self.file_type}"}
