...
try:
    ...
    if "phase1" in data and "files" in data["phase1"]:
        ...
        for file_id, file_data in data["phase1"]["files"].items():
            ...
            if (
                "Systematic Theology" in file_id
                or "Systematic Theology" in file_path
                or "Systematic Theology" in artifacts_path
            ):
                found = True
                print(f"\n📄 File ID: {file_id}")
                print(f"   Status: {file_data.get('status', 'unknown')}")
                print(
                    f"   Classification: {file_data.get('classification', 'unknown')}"
                )
    ...
    if "phase2" in data and "files" in data["phase2"]:
        ...
        for file_id, file_data in data["phase2"]["files"].items():
            ...
            if (
                "Systematic Theology" in file_id
                or "Systematic Theology" in extracted_path
            ):
                found = True
                print(f"\n📄 File ID: {file_id}")
                print(f"   Status: {file_data.get('status', 'unknown')}")
                ...
                print(
                    f"      Gibberish score: {file_data.get('gibberish_score', 'N/A')}"
                )
                print(
                    f"      Perplexity: {file_data.get('perplexity', 'N/A')}"
                )
                print(
                    f"      Language: {file_data.get('language', 'unknown')}"
                )
                print(
                    f"      Lang confidence: {file_data.get('lang_confidence', 'N/A')}"
                )
