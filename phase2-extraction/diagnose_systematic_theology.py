...
try:
    ...
    print(
        f"   Alphabetic ratio: {sum(c.isalpha() for c in sample) / len(sample):.1%}"
    )
    print(
        f"   Digit ratio: {sum(c.isdigit() for c in sample) / len(sample):.1%}"
    )
    print(
        f"   Space ratio: {sum(c.isspace() for c in sample) / len(sample):.1%}"
    )
    print(
        f"   Punctuation ratio: {sum(not c.isalnum() and not c.isspace() for c in sample) / len(sample):.1%}"
    )
    ...
    print(
        f"   Special unicode chars: {special_count} ({special_count/len(sample):.1%})"
    )

    # Check for excessive non-ASCII
    non_ascii = sum(ord(c) > 127 for c in sample)
    print(
        f"   Non-ASCII characters: {non_ascii} ({non_ascii/len(sample):.1%})"
    )
    ...
    if any(c in sample for c in ["", "", "", ""]):
        print(
            "   ⚠️  Contains private use area Unicode - Custom font mapping issue"
        )
