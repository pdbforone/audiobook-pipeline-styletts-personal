...
if tabs > 0:
    ...
    print("   This is why text looks spaced out.")
    print("   pypdf is extracting tabs instead of spaces.")
    print("\n   💡 Solution: Update TTS normalizer to handle tabs:")
    print("      text = text.replace('\\t', ' ')  # Convert tabs to spaces")
    print("      text = re.sub(r' {2,}', ' ', text)  # Then collapse")
else:
    print("   ✅ No tabs found")

    if multi_space > 10000:
        print(f"   ❌ But {multi_space:,} multiple-space sequences!")
        print("   Normalizer didn't run or failed.")
