...
to find which one produces readable text.
"""
import sys
from pathlib import Path

# Try importing all extraction libraries
try:
    import pdfplumber
...
if pdfplumber:
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            print(f"   Looks readable: {'✓' if any(word in sample.lower() for word in ['the', 'and', 'of', 'to', 'a']) else '✗'}")
...
if fitz:
    ...
    try:
        ...
        for page_num in range(min(2, len(doc))):
            ...
            # blocks returns a list of tuples, need to extract text
...
if fitz:
    ...
    try:
        doc = fitz.open(pdf_path)
        text = ""
        page = doc[0]  # First page only for this test
...
if partition:
    ...
    try:
        ...
        text = "\n".join(str(el) for el in elements[:10])  # First 10 elements only
...
for method, text in results.items():
    if text and len(text) > 100:
        ...
        common_words = sum(word in ['the', 'and', 'of', 'to', 'a', 'in', 'is', 'that', 'for', 'it'] for word in words)
...
print("\n💡 RECOMMENDATION:")
readable_methods = [m for m, t in results.items() if t and len(t) > 100 and 
                   sum(w in ['the', 'and', 'of', 'to', 'a'] for w in t[:1000].lower().split()) > 5]
if readable_methods:
    print(f"   Use: {readable_methods[0]}")
    print(f"   This method produced readable English text.")
else:
