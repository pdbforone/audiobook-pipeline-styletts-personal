...
try:
    ...
    logger.warning("TTS normalizer not available - text quality may be degraded")
...
def load_from_json(json_path: str, file_id: str, file_arg: str = None) -> Dict:
    try:
        ...
        # Smart fallback: default to 'text' if Phase 1 didn't run
        # This prevents slow OCR from being triggered unnecessarily
        classification = file_data.get("classification")
        if not classification:
            logger.warning(f"No classification found in Phase 1 - defaulting to 'text'")
            logger.warning(f"Run Phase 1 first for optimal performance!")
            classification = "text"
...
def extract_text_pypdf(file_path: str) -> str:
    ...
    WARNING: May produce tabs between words for some PDFs - normalize after!
    """
...
def extract_text_pymupdf(file_path: str) -> str:
    """Extract with PyMuPDF (fitz) - fast and reliable."""
    try:
        import pymupdf as fitz
...
def main(config: ExtractionConfig, file_arg: str = None):
    ...
    if not text.strip():
        ...
        # CRITICAL: Normalize text for TTS BEFORE saving
        if TTS_NORMALIZER_AVAILABLE:
            ...
            logger.warning("⚠️  TTS normalization skipped - install tts_normalizer.py for better quality")
