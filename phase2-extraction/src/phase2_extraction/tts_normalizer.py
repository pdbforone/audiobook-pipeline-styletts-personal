...
def normalize_for_tts(text: str) -> Tuple[str, dict]:
    ...
    if before > after:
        stats['changes'].append(f"Collapsed {before - after} multiple-space instances")
...
def validate_tts_readiness(text: str) -> Tuple[bool, list]:
    ...
    if multiple_spaces > 0:
        issues.append(f"Found {multiple_spaces} multiple-space instances (will cause TTS pauses)")
    ...
    if non_ascii_pct > 5:
        issues.append(f"High non-ASCII content ({non_ascii_pct:.1f}%) - may affect TTS")
...
if __name__ == "__main__":
    ...
    output_file = Path(sys.argv[2]) if len(sys.argv) > 2 else input_file.with_stem(f"{input_file.stem}_TTS_READY")
    ...
    # Print report
    print(f"\n{'='*60}")
    print(f"TTS Normalization Complete")
    print(f"{'='*60}")
