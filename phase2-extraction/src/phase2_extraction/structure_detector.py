...
def extract_pdf_structure_by_fonts(pdf_path: str, text: str) -> List[StructureNode]:
    """
    ...
    try:
        ...
        for item in font_data:
            if item["size"] >= threshold:
                ...
                is_mostly_caps = sum(c.isupper() for c in text_content if c.isalpha()) / max(1, len([c for c in text_content if c.isalpha()])) > 0.7
        ...
        unique_sizes = sorted(set(h["size"] for h in heading_candidates), reverse=True)
...
def detect_structure_heuristic(text: str) -> List[StructureNode]:
    ...
    for line_num, line in enumerate(lines):
        ...
        if not matched and line_stripped.isupper() and 5 <= len(line_stripped) <= 60:
...
def calculate_section_boundaries(nodes: List[StructureNode], text_length: int) -> List[StructureNode]:
    """
...
def extract_structure(pdf_path: str, text: str, enable_heuristic: bool = True) -> List[StructureNode]:
    """
