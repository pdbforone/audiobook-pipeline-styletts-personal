...
def extract(path: Path) -> Tuple[str, Dict]:
    ...
    try:
        ...
        if READABILITY_AVAILABLE:
            try:
                logger.debug(
                    "Using readability algorithm for content extraction"
                )
                ...
                logger.warning(
                    f"Readability extraction failed: {e}, falling back to basic extraction"
                )
        ...
        if text.count("\n") / len(text) > 0.1:
            logger.warning(
                "High line break density - HTML may have formatting issues"
            )
