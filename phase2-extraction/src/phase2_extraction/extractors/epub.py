...
def extract(path: Path) -> Tuple[str, Dict]:
    ...
    if not EPUB_AVAILABLE:
        error_msg = "ebooklib library not available. Install with: poetry add ebooklib"
    ...
    try:
        ...
        for item in book.get_items_of_type(epub.ITEM_DOCUMENT):
            ...
            try:
                ...
                if chapter_text.strip():
                    ...
                    chapter_count += 1
                    logger.debug(f"Extracted chapter {chapter_count}: {item_id} ({len(chapter_text)} chars)")
        ...
        if not text.strip():
            ...
            logger.warning("No chapters detected - EPUB may have unusual structure")
