...
def analyze_file(file_path: Path) -> Dict:
    ...
    analysis["non_ascii_pct"] = (
        (analysis["non_ascii"] / len(text) * 100) if text else 0
    )

    # Structure
    analysis["empty_lines"] = sum(1 for l in lines if not l.strip())
    analysis["empty_line_pct"] = (
        (analysis["empty_lines"] / len(lines) * 100) if lines else 0
    )
    ...
    analysis["ocean_pdf"] = len(
        re.findall(r"OceanofPDF\.com", text, re.IGNORECASE)
    )
...
def compare_text_samples(
    text1: str, text2: str, sample_size: int = 1000
) -> Dict:
    ...
    comparisons["middle"] = {
        ...
        "identical": text1[mid1 : mid1 + sample_size]
...
def find_differences(
    text1: str, text2: str, max_examples: int = 5
) -> List[Dict]:
            ...
            differences.append(
                {
                    ...
                    "text2_pos": f"{j1}-{j2}",
                    "text1_content": text1[
                        max(0, i1 - 30) : min(len(text1), i2 + 30)
                    ],
                    "text2_content": text2[
                        max(0, j1 - 30) : min(len(text2), j2 + 30)
                    ],
                }
            )
...
def compare_files(file1: Path, file2: Path):
    ...
    print(f"\n{'='*80}")
    print("🔎 SPECIFIC DIFFERENCES (First 5)")
    print(f"{'='*80}")
    ...
    if not differences:
        ...
        for i, diff in enumerate(differences, 1):
            ...
            print(
                f"  File 1: ...{diff['text1_content'][:100].replace(chr(10), '↵')}..."
            )
            print(
                f"  File 2: ...{diff['text2_content'][:100].replace(chr(10), '↵')}..."
            )
    ...
    if a1["multiple_spaces"] < a2["multiple_spaces"]:
        score1 += 10
        reasons.append(
            f"File 1 has fewer spacing issues ({a1['multiple_spaces']} vs {a2['multiple_spaces']})"
        )
    elif a2["multiple_spaces"] < a1["multiple_spaces"]:
        score2 += 10
        reasons.append(
            f"File 2 has fewer spacing issues ({a2['multiple_spaces']} vs {a1['multiple_spaces']})"
        )
    ...
    if a1["size"] > a2["size"]:
        ...
        reasons.append(
            f"File 1 is larger by {diff_pct:.1f}% ({a1['size'] - a2['size']:,} chars)"
        )
    elif a2["size"] > a1["size"]:
        score2 += 5
        diff_pct = (a2["size"] - a1["size"]) / a1["size"] * 100
        reasons.append(
            f"File 2 is larger by {diff_pct:.1f}% ({a2['size'] - a1['size']:,} chars)"
        )

    if a1["non_ascii_pct"] < a2["non_ascii_pct"]:
        score1 += 3
        reasons.append("File 1 has less non-ASCII content")
    elif a2["non_ascii_pct"] < a1["non_ascii_pct"]:
        score2 += 3
        reasons.append("File 2 has less non-ASCII content")
