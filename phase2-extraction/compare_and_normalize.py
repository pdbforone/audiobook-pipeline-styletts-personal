...
def count_tts_issues(text):
    ...
    issues['common_words_found'] = sum(1 for w in common_words if f' {w} ' in normalized_sample)
...
print(f"  Difference: {multipass['length'] - existing['length']:,} chars ({(multipass['length']/existing['length']-1)*100:.1f}% more)")
...
print(f"  Reduction: {len(existing['full_text']) - len(existing_normalized):,} chars")
...
print(f"  Reduction: {len(multipass['full_text']) - len(multipass_normalized):,} chars")
...
if multipass_score > existing_score:
    ...
    print(f"   More complete: +{multipass['length'] - existing['length']:,} chars")
    ...
    if multipass['length'] > existing['length']:
        ...
        print(f"   More text: +{multipass['length'] - existing['length']:,} chars")
        print(f"   File: {multipass_norm_file}")
        winner = "multipass"
    else:
