...
class MultiPassExtractor:
    ...
    def validate_extraction(self, text: str, method_name: str) -> Tuple[float, List[str]]:
        """
        ...
        logger.info(f"  {method_name} quality: {score:.2f} | Issues: {len(issues)}")
        return score, issues
    ...
    def compare_extractions(self) -> Dict[str, any]:
        ...
        # Find best by score
        best_method = max(scored_results.keys(), key=lambda k: scored_results[k]["score"])
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"EXTRACTION COMPARISON")
        logger.info(f"{'='*80}")
        for method, data in scored_results.items():
            marker = "👑" if method == best_method else "  "
            logger.info(f"{marker} {method:15s} | Score: {data['score']:.2f} | Length: {data['length']:,} | Issues: {len(data['issues'])}")
    ...
    def extract_with_consensus(self, min_confidence: float = 0.7) -> Tuple[str, Dict]:
        """
        ...
        if confidence >= 0.9:
            ...
            logger.info(f"⚠️  ACCEPTABLE QUALITY (confidence: {confidence:.2f})")
        ...
        metadata = {
            ...
            "all_results": {k: {"score": v["score"], "length": v["length"]} 
...
def extract_with_self_correction(file_path: str, min_confidence: float = 0.7) -> Tuple[str, Dict]:
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python multi_pass_extractor.py <pdf_file> [min_confidence]")
