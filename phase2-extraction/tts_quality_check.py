...
if replacement_chars > 0:
    ...
    print("   ✓ No replacement characters")
...
if private_use > 0:
    critical_failures.append(
        f"Found {private_use} private use area characters (font mapping error)"
    )
    print(
        f"   ❌ CRITICAL: {private_use} private use area characters (font mapping failed)"
    )
else:
    print("   ✓ No private use area characters")
...
if non_ascii_ratio > 0.15:  # Very strict - 15% max
    ...
    print(
        f"   ❌ CRITICAL: {non_ascii_ratio:.1%} non-ASCII characters (threshold: 15%)"
    )
...
if alpha_ratio < 0.65:  # Should be mostly letters
    ...
    print(
        f"   ❌ CRITICAL: Only {alpha_ratio:.1%} alphabetic characters (need >65%)"
    )
else:
...
if found_words < 8:
    critical_failures.append(
        f"Only found {found_words}/10 common English words"
    )
...
punct_per_100_words = (
    (periods + commas + questions + exclamations) / word_count * 100
)

if punct_per_100_words < 5:
    critical_failures.append(
        f"Low punctuation density: {punct_per_100_words:.1f} per 100 words"
    )
    print(
        f"   ❌ CRITICAL: {punct_per_100_words:.1f} punctuation marks per 100 words"
    )
    print("      (need 5+) - Will cause bad TTS phrasing")
elif punct_per_100_words < 10:
    warnings.append(
        f"Below-average punctuation: {punct_per_100_words:.1f} per 100 words"
    )
    print(
        f"   ⚠️  WARNING: {punct_per_100_words:.1f} punctuation marks per 100 words"
    )
else:
    print(f"   ✓ Punctuation density: {punct_per_100_words:.1f} per 100 words")
    print(
        f"      Periods: {periods}, Commas: {commas}, Questions: {questions}, Exclamations: {exclamations}"
    )
...
if len(sentences) < 10:
    critical_failures.append(
        f"Only {len(sentences)} sentences in first 20k chars"
    )
    ...
    if avg_sentence_length > 50:
        warnings.append(
            f"Very long sentences (avg {avg_sentence_length:.1f} words)"
        )
        print("      ⚠️  WARNING: Long sentences may cause TTS issues")
...
if not found_problem_chars:
    print("   ✓ No TTS-breaking special characters")
...
for word in words:
    ...
    if len(clean_word) >= 4:
        ...
        consonant_runs = re.findall(
            r"[bcdfghjklmnpqrstvwxyz]{5,}", clean_word.lower()
        )
        if consonant_runs:
...
if len(garbled_words) > 10:
    critical_failures.append(
        f"Found {len(garbled_words)} potentially garbled words"
    )
    ...
    print("   ✓ No obviously garbled words")
...
if critical_failures:
    ...
    print("\n💡 RECOMMENDATION:")
    print(
        "   This extraction has critical errors that WILL cause TTS hallucinations."
    )
    ...
    print("\n💡 RECOMMENDATION:")
    print(
        "   Text is readable but has minor issues that might affect TTS quality."
    )
