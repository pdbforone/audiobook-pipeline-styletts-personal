...
def analyze_text_quality(file_path, label):
    ...
    if multiple_spaces > 0:
        print(f"    ❌ BAD - Will cause TTS pauses")
        # Show example
    ...
    if len(short_lines) > 100:
        print(f"    ⚠️  Many short lines might indicate poor extraction")
    
    # Empty lines ratio
    empty_lines = sum(1 for l in lines if not l.strip())
    print(f"  Empty lines: {empty_lines:,} ({empty_lines/len(lines)*100:.1f}%)")
...
def compare_sample_pages(file1, file2):
    ...
    print(f"\n{'='*60}")
    print(f"Sample Comparison (first 1000 chars)")
    print(f"{'='*60}")
...
def main():
    ...
    orch_metrics = analyze_text_quality(orch_file, "ORCHESTRATOR OUTPUT (Hours Run)")
    ...
    if orch_metrics['multiple_spaces'] > test_metrics['multiple_spaces']:
        diff = orch_metrics['multiple_spaces'] - test_metrics['multiple_spaces']
        issues.append(f"❌ Orchestrator has {diff:,} MORE multiple-space issues")
        issues.append(f"   → This will cause bad TTS pauses!")
    
    if test_metrics['multiple_spaces'] > orch_metrics['multiple_spaces']:
        diff = test_metrics['multiple_spaces'] - orch_metrics['multiple_spaces']
        issues.append(f"❌ Test has {diff:,} MORE multiple-space issues")
    
    if abs(test_metrics['paragraphs'] - orch_metrics['paragraphs']) > 100:
        issues.append(f"⚠️  Different paragraph counts: Test={test_metrics['paragraphs']}, Orch={orch_metrics['paragraphs']}")
    ...
    print(f"\n💡 Recommendation:")
    if test_metrics['multiple_spaces'] < orch_metrics['multiple_spaces']:
        print(f"  → Use TEST output (fewer spacing issues)")
        print(f"  → File: {test_file.name}")
    elif orch_metrics['multiple_spaces'] < test_metrics['multiple_spaces']:
        print(f"  → Use ORCHESTRATOR output (fewer spacing issues)")
        print(f"  → File: {orch_file.name}")
    else:
        print(f"  → Both files have similar quality - use either")
