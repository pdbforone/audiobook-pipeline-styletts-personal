...
def test_dependencies():
    ...
    optional_deps = [
        ...
        (
            "readability",
            "readability-lxml - HTML content extraction (optional)",
        ),
...
def test_extractors():
    ...
    for name, module in extractors:
        try:
            ...
            assert hasattr(
                module, "extract"
            ), f"{name}: Missing extract() function"
            ...
            assert (
                "path" in params
            ), f"{name}: extract() should have 'path' parameter"
...
def main():
    ...
    if all_passed:
        ...
        print(
            "     poetry run python -m phase2_extraction.ingest --file_id test001 --file /path/to/test.pdf"
        )
        ...
        print(
            "  poetry add python-docx ebooklib beautifulsoup4 lxml easyocr pdf2image pypdf"
        )
        print("\nOn Windows, also run:")
