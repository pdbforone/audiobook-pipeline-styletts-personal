...
class ConsensusExtractor:
    ...
    def choose_best_page_text(
        self, page_num: int, extractions: Dict[str, str]
    ) -> Tuple[str, str, float]:
        """
    ...
    def extract_with_consensus(
        ...
        logger.info(
            f"Pages: {self.page_count} | Methods: {', '.join(methods)}"
        )
        logger.info(f"{'='*80}\n")
        ...
        for page_num in range(self.page_count):
            ...
            best_text, best_method, confidence = self.choose_best_page_text(
                page_num, page_extractions
            )

            if confidence < 0.3:
                failed_pages.append(page_num)
                logger.warning(
                    f"  Page {page_num+1}: FAILED (confidence: {confidence:.2f})"
                )
            elif confidence < min_confidence:
                low_confidence_pages.append(page_num)
                logger.info(
                    f"  Page {page_num+1}: LOW (confidence: {confidence:.2f}) via {best_method}"
                )
            else:
                logger.debug(
                    f"  Page {page_num+1}: OK (confidence: {confidence:.2f}) via {best_method}"
                )
            ...
            if (page_num + 1) % 10 == 0:
                logger.info(
                    f"  Processed {page_num + 1}/{self.page_count} pages..."
                )
        ...
        if page_metadata:
            avg_confidence = sum(p["confidence"] for p in page_metadata) / len(
                page_metadata
            )
        else:
            avg_confidence = 0.0
        ...
        if low_confidence_pages:
            logger.warning(
                f"Low confidence pages: {len(low_confidence_pages)}"
            )
            logger.warning(
                f"  Pages: {', '.join(str(p+1) for p in low_confidence_pages[:10])}"
            )

        if failed_pages:
            logger.error(f"Failed pages: {len(failed_pages)}")
            logger.error(
                f"  Pages: {', '.join(str(p+1) for p in failed_pages[:10])}"
            )
    ...
    def extract_failed_pages_with_ocr(
...
def extract_with_consensus(
    ...
    if use_ocr_fallback and metadata["failed_pages"]:
        ...
        ocr_text = extractor.extract_failed_pages_with_ocr(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python consensus_extractor.py <pdf_file> [min_confidence]"
        )
        sys.exit(1)
