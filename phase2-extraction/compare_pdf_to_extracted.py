...
def compare_page_to_extracted(pdf_path, txt_path, page_num):
    ...
    if overlap > 10:
        print("   ✅ Good overlap - extraction seems accurate")
    else:
        print("   ⚠️  Low overlap - may not be same section")
...
def search_in_both(pdf_path, txt_path, search_term):
    ...
    for page_num in range(
        min(page_count, 100)
    ):  # Limit search to first 100 pages
    ...
    if found_pages:
        print(
            f"✅ Found in PDF on pages: {', '.join(map(str, found_pages[:10]))}"
        )
        if len(found_pages) > 10:
            print(f"   (and {len(found_pages) - 10} more pages)")
    else:
        print("❌ NOT found in PDF (searched first 100 pages)")
