...
def analyze_file(file_path):
    ...
    print("\n📊 Basic Stats:")
    print(
        f"  Size: {len(content):,} characters ({len(content)/1024/1024:.2f} MB)"
    )
    ...
    print(
        f"  Very long lines (>500 chars): {sum(1 for line in content.split(chr(10)) if len(line) > 500):,}"
    )
    print(
        f"  Very short lines (<10 chars): {sum(1 for line in content.split(chr(10)) if 0 < len(line) < 10):,}"
    )
    print(
        f"  Lines with only numbers: {sum(1 for line in content.split(chr(10)) if line.strip().isdigit()):,}"
    )
    ...
    print(
        f"  Avg word length: {sum(len(w) for w in words) / len(words) if words else 0:.1f}"
    )
...
def main():
    ...
    # Compare them
    find_differences(
        test_content, orch_content, "TEST (TTS_READY)", "ORCHESTRATOR"
    )
