...
def score_quality(text):
    ...
    common = sum(1 for w in ['the', 'and', 'of', 'to', 'a'] if f' {w} ' in text_lower)
    if common < 3:
        score -= 0.2
...
def compare_extraction_methods(pdf_path, page_nums):
    ...
    available_methods = [name for name, func in methods.items() 
                         if (name == 'pypdf' and PYPDF) or 
                            (name == 'pdfplumber' and PDFPLUMBER) or 
                            (name == 'pymupdf' and PYMUPDF)]
    ...
    for method_name in available_methods:
        successful = sum(1 for v in results[method_name].values() if v is not None)
        if successful > 0:
            avg_length = sum(v['length'] for v in results[method_name].values() if v) / successful
            avg_quality = sum(v['quality'] for v in results[method_name].values() if v) / successful
...
if __name__ == "__main__":
    ...
    if choice == '1':
        ...
        pages_str = input("Enter page numbers (comma-separated, e.g. 1,5,10): ").strip()
        try:
