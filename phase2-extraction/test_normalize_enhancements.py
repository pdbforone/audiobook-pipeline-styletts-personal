...
from phase2_extraction.normalize import (
    normalize_text,
...
"""
    ...
    normalized, metrics = normalize_text(
        test_text, "test_inline", Path("extracted_text")
    )
    ...
    print("\nMetrics:")
    print(
        f"  Removed inline numbers: {metrics.get('removed_inline_numbers', 0)}"
    )
    print(
        f"  Converted numbers to words: {metrics.get('converted_numbers_to_words', 0)}"
    )
...
"""
    ...
    normalized, metrics = normalize_text(
        sample_text, "test_analects", Path("extracted_text")
    )
    ...
    if (
        "Three The Master" in normalized
        or "three The Master" in normalized.lower()
    ):
    ...
    if metrics.get("removed_inline_numbers", 0) > 0:
        print(
            f"✓ PASS - Removed {metrics['removed_inline_numbers']} inline numbers"
        )
    else:
