...
check_pypdf = subprocess.run(
    [
        ...
        "python",
        "-c",
        "import pypdf; print('pypdf installed')",
    ],
    cwd=str(phase2_dir),
    capture_output=True,
    text=True,
...
if "pypdf installed" not in check_pypdf.stdout:
    ...
    if install_result.returncode == 0:
        ...
        print(
            "⚠️  Could not install pypdf automatically. Install manually with:"
        )
...
try:
    ...
    print(
        f"   Language: {phase2_data.get('language', 'unknown')} (confidence: {phase2_data.get('lang_confidence', 0):.2f})"
    )
