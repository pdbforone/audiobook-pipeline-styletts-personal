...
class AudioDownloader:
    ...
    def extract_segment(
        ...
        try:
            logger.info(
                f"Extracting {duration}s segment starting at {start}s..."
            )
            cmd = [
                "ffmpeg",
                "-i",
    ...
    def trim_silence(self, input_path: Path, output_path: Path) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    f"Silence trimming failed (continuing anyway): {result.stderr}"
                )
    ...
    def validate_sample(self, audio_path: Path) -> dict:
        ...
        try:
            ...
            cmd = [
                ...
                "error",
                "-show_entries",
                "stream=sample_rate,channels,bits_per_sample,duration",
                "-of",
                "json",
                str(audio_path),
            ]
            ...
            if not data.get("streams"):
                return {"valid": False, "issues": ["No audio stream found"]}
            ...
            if duration < 10:
                issues.append(
                    f"Duration too short: {duration:.1f}s (minimum 10s)"
                )
            elif duration > 35:
                issues.append(
                    f"Duration too long: {duration:.1f}s (optimal 10-30s)"
                )
            elif duration < 15:
                issues.append(
                    f"Duration low: {duration:.1f}s (recommended 20-30s)"
                )

            if sample_rate < 24000:
                issues.append(
                    f"Sample rate low: {sample_rate}Hz (recommended 44100Hz+)"
                )
            ...
            if bit_depth != 16:
                issues.append(
                    f"Bit depth: {bit_depth}-bit (recommended 16-bit)"
                )
            ...
            result_stats = subprocess.run(
                cmd_stats, capture_output=True, text=True
            )

            # Parse max volume from stderr
            import re

            max_volume_match = re.search(
                r"max_volume: ([-\d.]+) dB", result_stats.stderr
            )
            if max_volume_match:
                max_volume = float(max_volume_match.group(1))
                if max_volume > -0.5:
                    issues.append(
                        f"Possible clipping: max volume {max_volume:.1f}dB (should be < -1dB)"
                    )

            # Check for silence (mean volume too low)
            mean_volume_match = re.search(
                r"mean_volume: ([-\d.]+) dB", result_stats.stderr
            )
            if mean_volume_match:
                mean_volume = float(mean_volume_match.group(1))
                if mean_volume < -30:
                    issues.append(
                        f"Audio very quiet: mean volume {mean_volume:.1f}dB"
                    )
    ...
    def download_narrator(
        self, genre: str, narrator: str, skip_validation: bool = False
    ) -> bool:
        ...
        if not narrator_data.get("commercial_use", False):
            ...
            logger.warning(
                f"License: {narrator_data.get('license', 'Unknown')}"
            )
            logger.warning(
                f"Note: {narrator_data.get('note', 'Verify commercial usage rights')}"
            )
            return False
        ...
        for i, url in enumerate(narrator_data["urls"], 1):
            ...
            segment_info = narrator_data.get(
                "extract_segment", {"start": 0, "duration": 25}
            )
            temp_filename = f"{narrator.replace(' ', '_')}_{i:02d}_temp.wav"
            temp_path = narrator_dir / temp_filename

            processed_filename = (
                f"{narrator.replace(' ', '_')}_{i:02d}_sample.wav"
            )
            ...
            if not processed_path.exists():
                ...
                if not skip_validation:
                    ...
                    logger.info(f"{'='*60}")
                    logger.info(
                        f"Duration: {validation.get('duration', 0):.1f}s"
                    )
                    logger.info(
                        f"Sample Rate: {validation.get('sample_rate', 0)}Hz"
                    )
                    logger.info(f"Channels: {validation.get('channels', 0)}")
                    logger.info(
                        f"Bit Depth: {validation.get('bit_depth', 0)}-bit"
                    )
        ...
        logger.info(
            f"Successfully processed {success_count}/{len(narrator_data['urls'])} samples"
        )
        logger.info(
            "✓ All samples are Public Domain (LibriVox) - commercially usable"
        )
    ...
    def list_narrators(self):
        ...
        print(
            "NOTE: Using Book/Chapter 2-3 to avoid translator prefaces in Book 1\n"
        )
        ...
        for genre, narrators in VOICE_CATALOG.items():
            ...
            for narrator, data in narrators.items():
                ...
                if data.get("extract_segment"):
                    ...
                    print(
                        f"     Sample: {seg['duration']}s extracted from {seg['start']}s mark"
                    )
...
def main():
    ...
    parser.add_argument("--narrator", help="Specific narrator name")
    parser.add_argument(
        "--download-all",
        action="store_true",
    ...
    if args.genre and args.narrator:
        success = downloader.download_narrator(
            args.genre, args.narrator, skip_validation=args.skip_validation
        )
        ...
        for genre, narrators in VOICE_CATALOG.items():
            for narrator in narrators:
                ...
                if downloader.download_narrator(
                    genre, narrator, skip_validation=args.skip_validation
                ):
                    success_count += 1
