...
def prepare_reference_audio(
    ...
    ta.save(
        output_path, torch.tensor(y_resampled).unsqueeze(0), config.sample_rate
    )
    os.remove(mp3_path)
    return output_path
...
def synthesize_chunk(
    ...
    if (
        config.enable_splitting and len(text) > 300
    ):  # Threshold for splitting (adjust per model limits)
    ...
    )  # Initialize as 2D empty [channels=1, samples=0]
    for i, sub_text in enumerate(sub_chunks):
        try:
            ...
            logger.info(
                f"Before concat: full_audio={full_audio.shape}, wav={wav.shape}"
            )
            ...
            logger.info(
                f"Before concat silence: full_audio shape={full_audio.shape}, silence shape={silence.shape}"
            )
            full_audio = (
                torch.cat([full_audio, silence], dim=1)
            ...
            for _ in range(config.sub_chunk_retries - 1):  # Retries
                try:
                    ...
                    if wav.dim() == 1:
                        wav = wav.unsqueeze(0)
                    logger.info(
                        f"Retry success: replacing silence with wav={wav.shape}"
                    )
                    full_audio = torch.cat(
                        [full_audio[:, :-silence_samples], wav], dim=1
                    )  # Replace silence (slice time dim)
                    break
                except Exception as retry_e:
                    logger.warning(
                        f"Retry failed for sub-chunk {i}: {retry_e}"
                    )
...
def merge_to_pipeline_json(
    ...
    data["phase4"]["status"] = (
        ...
        if all(
            r["status"] == "success"
            for r in data["phase4"]["files"][file_id].values()
        )
        else "partial"
    )
