...
def main():
    ...
    if not phase4_audio_dir.exists():
        console.print(f"[red]❌ Phase 4 audio directory not found: {phase4_audio_dir}[/red]")
        return 1
    
    audio_files = list(phase4_audio_dir.glob("*.wav"))
    console.print(f"[green]✓[/green] Found {len(audio_files)} audio files in Phase 4\n")
    ...
    console.print(f"[green]✓[/green] Config updated (backup saved to config.yaml.backup)")
    console.print(f"   - resume_on_failure: {config['resume_on_failure']}")
    console.print(f"   - quality_validation: {config['quality_validation_enabled']}")
    console.print(f"   - input_dir: {config['input_dir']}\n")
    
    # Clear Phase 5 data from pipeline.json
    console.print("[yellow]📝 Clearing old Phase 5 data from pipeline.json...[/yellow]")
    ...
    try:
        ...
        if 'phase5' in pipeline_data:
            ...
            console.print(f"[green]✓[/green] Cleared {old_count} old Phase 5 entries\n")
        else:
            console.print(f"[green]✓[/green] No old Phase 5 data to clear\n")
    except Exception as e:
        console.print(f"[yellow]⚠️[/yellow]  Could not clear Phase 5 data: {e}\n")
    ...
    if processed_dir.exists():
        ...
        if old_files:
            console.print(f"[yellow]📝 Clearing {len(old_files)} old files from processed/...[/yellow]")
            shutil.rmtree(processed_dir)
            processed_dir.mkdir(parents=True, exist_ok=True)
            console.print(f"[green]✓[/green] Cleared processed directory\n")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            console.print(f"[yellow]Check the logs above for errors[/yellow]")
            return 1
        ...
        console.print(f"[green]Processed files:[/green] {len(processed_files)}/{len(audio_files)}")
        
        if audiobook.exists():
            size_mb = audiobook.stat().st_size / (1024 * 1024)
            console.print(f"[green]Final audiobook:[/green] {audiobook} ({size_mb:.1f} MB)")
            console.print(f"\n[bold cyan]✓ Success! Listen to: {audiobook}[/bold cyan]\n")
        else:
            console.print(f"[yellow]⚠️  audiobook.mp3 not found (chunks may have failed)[/yellow]\n")
        
        if len(processed_files) < len(audio_files):
            missing = len(audio_files) - len(processed_files)
            console.print(f"[yellow]⚠️  Warning: {missing} chunks may have failed[/yellow]")
            console.print(f"[yellow]   Run analyze_phase5_failures.py to diagnose[/yellow]\n")
        ...
        if config_backup.exists():
            ...
            console.print(f"[green]✓[/green] Restored original config.yaml")
