...
pattern1_files = [
    f for f in all_files if f.stem.startswith(f"{file_id}_chunk_")
]
pattern2_files = [
    f
    for f in all_files
    if f.stem.startswith("chunk_") and not f.stem.startswith(f"{file_id}_")
]
...
if pattern1_files:
    ...
    print("Using filename pattern: chunk_NNN.wav")
...
try:
    ...
    pipeline["phase4"]["files"][file_id]["audio_dir"] = str(
        phase4_audio_dir.resolve()
    )
    ...
    print(
        f"✓ Updated pipeline.json with {len(audio_paths)} ABSOLUTE audio paths"
    )
    print()
    print("=" * 60)
    ...
    print(
        f"  poetry run python orchestrator.py \"{project_root / 'input' / 'The_Analects_of_Confucius_20240228.pdf'}\" --phases 5 --no-resume"
    )
