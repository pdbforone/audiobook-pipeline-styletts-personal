...
def summarize_pipeline_json(pipeline_json_path):
    ...
    try:
        ...
        # Write summary
        summary_path = (
            Path(pipeline_json_path).parent / "pipeline_summary.json"
        )
        ...
        logger.info(
            f"Pipeline.json size: {summary['total_size_bytes']:,} bytes"
        )
...
def run_phase4_chunks(phase_dir, conda_env, file_id, pipeline_json):
    ...
    try:
        ...
        if not chunks and phase3_files:
            logger.warning(
                f"Exact file_id '{file_id}' not found in Phase 3 output"
            )
            logger.info(
                f"Available keys in Phase 3: {list(phase3_files.keys())}"
            )
        ...
        if not chunks:
            ...
            logger.error(
                f"Available Phase 3 files: {list(phase3_files.keys())}"
            )
            return False
        ...
        if chunks != phase3_files[file_id].get("chunk_paths", []):
            ...
            if verify_chunks and Path(verify_chunks[0]).is_absolute():
                ...
                logger.warning(
                    "⚠ Verification failed: Paths may still be relative"
                )
    ...
    for i in range(len(chunks)):
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                logger.warning(
                    f"  Chunk {i} failed with exit code {result.returncode}"
                )
                logger.warning(
                    f"  Error: {error_lines[-1] if error_lines else 'Unknown error'}"
                )
