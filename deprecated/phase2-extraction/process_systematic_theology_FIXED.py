...
try:
    ...
    if result.returncode != 0:
        ...
        print("\nTroubleshooting:")
        print(
            "  1. Check if pypdf is installed: poetry run pip list | findstr pypdf"
        )
        print(
            "  2. Try running test_extraction_methods.py to test all methods"
        )
...
try:
    ...
    print(
        f"   Gibberish score: {phase2_data.get('gibberish_score', 'N/A'):.3f}"
    )
    print(f"   Perplexity: {phase2_data.get('perplexity', 'N/A'):.3f}")
    print(
        f"   Language: {phase2_data.get('language', 'unknown')} (confidence: {phase2_data.get('lang_confidence', 0):.3f})"
    )
    ...
    if extracted_path:
        ...
        if extracted_path.exists():
            ...
            if gibberish > 0.7:
                ...
                print(
                    "      3. Try OCR extraction (Phase 2 with --classification=scanned)"
                )
            elif gibberish > 0.5:
                print(
                    "   ⚠️  MODERATE GIBBERISH (0.5-0.7) - Some quality issues"
                )
                print(
                    "      Text may be partially readable but has encoding problems"
                )
            elif perplexity < 0.5:
                print("   ⚠️  LOW PERPLEXITY (<0.5) - Low vocabulary diversity")
                print(
                    "      May indicate repeated text or extraction artifacts"
                )
            elif lang_conf < 0.8:
                print(
                    "   ⚠️  LOW LANGUAGE CONFIDENCE - May not be English or mixed content"
                )
            else:
