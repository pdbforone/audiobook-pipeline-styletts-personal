...
try:
    ...
    logger.warning(
        "TTS normalizer not available - text quality may be degraded"
    )
...
def load_from_json(json_path: str, file_id: str, file_arg: str = None) -> Dict:
    try:
        ...
        if not classification:
            logger.warning(
                "No classification found in Phase 1 - defaulting to 'text'"
            )
...
def main(config: ExtractionConfig, file_arg: str = None):
    ...
    if not text.strip():
        ...
        if TTS_NORMALIZER_AVAILABLE:
            ...
            logger.warning(
                "⚠️  TTS normalization skipped - install tts_normalizer.py for better quality"
            )
