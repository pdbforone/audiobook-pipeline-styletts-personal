...
class ExtractionConfig(BaseModel):
    ...
    tts_normalize: bool = True  # NEW: Enable TTS normalization
...
class ExtractionRecord(BaseModel):
    ...
    tts_ready: Optional[bool] = None  # NEW: TTS readiness flag
    tts_metrics: Optional[Dict] = None  # NEW: TTS quality metrics
...
def extract_text_pypdf(file_path: str) -> str:
    """Extract text using pypdf (best for font encoding)."""
...
def extract_text_easyocr(file_path: str) -> str:
    try:
        ...
        doc.close()
        logger.info(f"OCR complete: {len(text)} characters")
        return text
    except Exception as e:
...
def main(config: ExtractionConfig, file_arg: str = None):
    ...
    while retries <= config.retry_limit:
        if classification == "text" or classification == "mixed":
            # Try pypdf FIRST - best for fonts
    ...
    if not text.strip():
        ...
        # NEW: TTS Normalization (CRITICAL for Phase 4)
        raw_length = len(text)
