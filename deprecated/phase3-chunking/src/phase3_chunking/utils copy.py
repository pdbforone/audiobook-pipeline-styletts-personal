...
MAX_CHUNK_CHARS = (
    2000  # ~25s at 750 cpm (MAX_DURATION_SECONDS / 60 * CHARS_PER_MINUTE)
)
...
MAX_DURATION_SECONDS = (
    25  # Target maximum duration to avoid Chatterbox 40s cutoff
)
...
def split_long_chunk(
    chunk: str, max_chars: int, max_duration: float
) -> List[str]:
    """
    ...
    def recursive_split(text: str) -> List[str]:
        ...
        if len(words) == 1:
            ...
            logger.warning(
                f"Single word exceeds limits: {len(text)} chars, {current_duration:.1f}s"
            )
        ...
        for word in words:
            ...
            if (
                current_sub_len + word_len > max_chars
                or test_duration > max_duration
            ) and current_sub:
...
def _chunk_by_char_count_optimized(
    ...
    for sent in sentences:
        ...
        if sent_len > max_chars or sent_duration > max_duration:
            ...
            if current_chunk and current_char_count >= min_chars:
                ...
                if (
                    len(chunk_text) <= max_chars
                    and chunk_duration <= max_duration
                ):
                    chunks.append(chunk_text)
                else:
                    # 🔧 FIX: Recursively split if still too long
                    chunks.extend(
                        split_long_chunk(chunk_text, max_chars, max_duration)
                    )
        ...
        if test_len > max_chars or test_duration > max_duration:
            ...
            if current_chunk and current_char_count >= min_chars:
                ...
                if current_chunk:
                    ...
                    chunks.extend(
                        split_long_chunk(test_text, max_chars, max_duration)
                    )
                else:
                    # Just this sentence, already handled above
                    chunks.extend(
                        split_long_chunk(sent, max_chars, max_duration)
                    )
    ...
    if current_chunk and current_char_count >= min_chars:
        ...
        if len(final_text) <= max_chars and final_duration <= max_duration:
            ...
            chunks.extend(
                split_long_chunk(final_text, max_chars, max_duration)
            )
    elif current_chunk:
        ...
        if len(final_text) >= 50:  # Keep if at least 50 chars
            ...
            if chunks:
                ...
                if (
                    len(test_merged) <= max_chars
                    and test_duration <= max_duration
                ):
                    chunks[-1] = test_merged
                else:
                    chunks.append(
                        final_text
                    )  # Add as-is, will be handled in merge
            else:
                chunks.append(final_text)
...
def form_semantic_chunks(
    ...
    chunks = _chunk_by_char_count_optimized(
        valid_sentences, min_chars, max_chars, max_duration
    )
    ...
    if len(chunks) > len(valid_chunks):
        logger.warning(
            f"Dropped {len(chunks) - len(valid_chunks)} invalid/short chunks after merging"
        )
    ...
    if oversized:
        ...
        for idx, dur in oversized[:3]:
            logger.error(
                f"   Chunk {idx+1}: {len(valid_chunks[idx])} chars, {dur:.1f}s"
            )
    else:
        logger.info(
            f"✅ SUCCESS: All {len(valid_chunks)} chunks are <= {max_duration}s"
        )
    ...
    if avg_coherence < 0.87:
        logger.warning(
            f"Low coherence ({avg_coherence:.4f}), checking Jaccard fallback"
        )
        jaccard_scores = []
        ...
        if avg_jaccard > 0.4:
            logger.info(
                f"Jaccard similarity ({avg_jaccard:.4f}) acceptable, keeping chunks"
            )
        else:
            logger.warning(
                f"Both coherence ({avg_coherence:.4f}) and Jaccard ({avg_jaccard:.4f}) are low"
            )
...
def log_chunk_times(chunks: List[str]):
    ...
    for i, chunk in enumerate(chunks):
        ...
        if (
            MIN_CHUNK_CHARS <= char_count <= MAX_CHUNK_CHARS
...
def calculate_chunk_metrics(chunks: List[str]) -> Dict[str, any]:
    ...
    chunks_in_target = sum(
        1
        for c, d in zip(char_lengths, durations)
        if MIN_CHUNK_CHARS <= c <= MAX_CHUNK_CHARS
        and d <= MAX_DURATION_SECONDS
    )
