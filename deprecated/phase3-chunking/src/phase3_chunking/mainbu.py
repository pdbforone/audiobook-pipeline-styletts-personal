...
def _merge_to_json_impl(
    ...
    valid_files = [f for f in all_files.values() if isinstance(f, dict) and "status" in f]
    if len(valid_files) < len(all_files):
        invalid_keys = [k for k, f in all_files.items() if not (isinstance(f, dict) and "status" in f)]
        logger.warning(f"Found {len(all_files) - len(valid_files)} malformed file entries in pipeline.json:phase3:files: {invalid_keys}")
    ...
    data["phase3"]["metrics"] = {
        ...
        "total_chunks": sum(len(f.get("chunk_paths", [])) for f in valid_files),
...
def main():
    ...
    try:
        ...
        print(f"Average chunk size: {metrics.get('avg_char_length', 0):.0f} chars, "
              f"{metrics.get('avg_word_count', 0):.0f} words")
        print(f"Average duration: {metrics.get('avg_chunk_duration', 0):.1f}s "
