...
def find_monorepo_root(start_path: Path) -> Path:
    ...
    # Walk up directory tree looking for phase directories
    ...
    raise FileNotFoundError(
        "Monorepo root not found. Set MONOREPO_ROOT environment variable or "
        "ensure phase directories exist in parent path."
    )
...
def load_structure_from_json(json_path: str, file_id: str):
    ...
    Returns:
        List of structure nodes or None if not available
    """
    try:
        ...
        if structure:
            logger.info(
                f"Loaded structure metadata from Phase 2: {len(structure)} nodes"
            )
            return structure
        else:
...
def process_chunking(
    ...
    Args:
        ...
        json_path: Path to pipeline.json for loading structure
        file_id: File ID for structure lookup

    Returns:
        ChunkRecord with chunking results (including chunk metrics)
    ...
    # Try to load structure from Phase 2
    structure = None
    if file_id:
        structure = load_structure_from_json(json_path, file_id)

    # Detect sentences (needed for fallback)
    ...
    # Form chunks - use structure if available, otherwise use semantic chunking
    if structure and should_use_structure_chunking(structure):
        ...
        chunks, coherence, embeddings = chunk_by_structure(
            ...
            structure,
            config,
            max_chunk_words=5000,  # Larger chunks to prevent Phase 4 timeout
        )
    else:
        logger.info("Using semantic chunking with character optimization")
        chunks, coherence, embeddings = form_semantic_chunks(
            sentences, config.min_chunk_words, config.max_chunk_words
        )

    if not chunks:
        raise ValueError("No chunks created from text")

    # NEW: Calculate detailed chunk metrics (sizes, durations)
    chunk_metrics = calculate_chunk_metrics(chunks)
    logger.info(
        f"Average chunk: {chunk_metrics['avg_char_length']:.0f} chars, "
    ...
    if avg_coherence < config.coherence_threshold:
        errors.append(
            f"Low coherence: {avg_coherence:.4f} < {config.coherence_threshold}"
        )
    if avg_flesch < config.flesch_threshold:
        errors.append(
            f"Low readability: {avg_flesch:.2f} < {config.flesch_threshold}"
        )

    # Check for duration warnings
    if chunk_metrics["max_duration"] > config.max_chunk_duration:
        errors.append(
            f"Some chunks exceed {config.max_chunk_duration}s duration "
            f"(max: {chunk_metrics['max_duration']:.1f}s), may require Phase 4 splitting"
        )
    ...
    # Create record with chunk metrics
    record = ChunkRecord(
        ...
        chunk_metrics=chunk_metrics,  # NEW: Include chunk size/duration metrics
    )
...
def _load_from_json_impl(
    json_path_abs: Path, file_id: str, strict: bool = False
) -> str:
    ...
    try:
        if not json_path_abs.exists():
            raise FileNotFoundError(
                f"Pipeline JSON not found: {json_path_abs}"
            )
        ...
        if not text_path:
            raise ValueError(
                "Phase 2 data missing 'extracted_text_path' field"
            )

        text_path_obj = Path(text_path)
        if not text_path_obj.exists():
            raise FileNotFoundError(
                f"Text file from Phase 2 not found: {text_path}"
            )
...
def merge_to_json(
    ...
    if FileLock:
        with FileLock(lock_file, timeout=10):
            _merge_to_json_impl(
                record, json_path_abs, file_id, fallback_used, fallback_message
            )
    else:
        _merge_to_json_impl(
            record, json_path_abs, file_id, fallback_used, fallback_message
...
def _merge_to_json_impl(
    ...
    # Filter valid files with 'status' key
    valid_files = [
        f for f in all_files.values() if isinstance(f, dict) and "status" in f
    ]
    if len(valid_files) < len(all_files):
        invalid_keys = [
            k
            for k, f in all_files.items()
            if not (isinstance(f, dict) and "status" in f)
        ]
        logger.warning(
            f"Found {len(all_files) - len(valid_files)} malformed file entries in pipeline.json:phase3:files: {invalid_keys}"
        )
        data["phase3"]["errors"].append(
            {
                "type": "MalformedData",
                "message": f"Invalid file entries missing 'status': {invalid_keys}",
                "timestamp": time.time(),
            }
        )
    ...
    data["phase3"]["metrics"] = {
        ...
        "successful": sum(1 for f in valid_files if f["status"] == "success"),
        "partial": sum(1 for f in valid_files if f["status"] == "partial"),
        "failed": sum(1 for f in valid_files if f["status"] == "failed"),
        "total_chunks": sum(
            len(f.get("chunk_paths", [])) for f in valid_files
        ),
        "avg_coherence": metrics.get("avg_coherence", 0),
        "avg_flesch": metrics.get("avg_flesch", 0),
    }

    try:
        lock_path = json_path_abs.with_suffix(".lock")
...
def load_config(config_path: str) -> ValidationConfig:
    ...
    return ValidationConfig(
        ...
        min_chunk_chars=config_data.get("min_chunk_chars", 400),
        max_chunk_chars=config_data.get("max_chunk_chars", 600),
...
def main():
    ...
    if args.text_path:
        ...
        try:
            text_path = load_from_json(
                args.json_path, args.file_id, args.strict
            )
        except Exception as e:
            fallback_used = True
    ...
    try:
        ...
        print(
            f"Average chunk size: {metrics.get('avg_char_length', 0):.0f} chars, "
