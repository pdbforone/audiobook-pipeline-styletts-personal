...
def enhance_chunk(
    ...
    try:
        ...
        if phrase_cleaner and config.enable_phrase_cleanup:
            logger.info(
                f"🧹 Running phrase cleanup on chunk {metadata.chunk_id}..."
            )
...
def main():
    ...
    try:
        ...
        try:
            ...
            if config.enable_phrase_cleanup:
                logger.info(
                    f"🧹 Phrase cleanup: {phrases_cleaned_total} phrases removed from {chunks_with_phrases} chunks"
                )
                if cleanup_errors > 0:
                    logger.warning(
                        f"⚠️  Cleanup errors: {cleanup_errors} chunks had cleanup failures"
                    )
            logger.info("=" * 60)
