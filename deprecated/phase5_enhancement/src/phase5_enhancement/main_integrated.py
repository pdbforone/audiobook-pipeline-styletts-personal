...
def enhance_chunk(
    ...
    try:
        ...
        if phrase_cleaner and config.enable_phrase_cleanup:
            logger.info(f"🧹 Running phrase cleanup on chunk {metadata.chunk_id}...")
            ...
            if cleaned_audio is not None:
                ...
                logger.info(
                    f"✓ Removed {metadata.phrases_removed} phrase(s) from chunk {metadata.chunk_id}"
                )
                audio = cleaned_audio
...
def main():
    ...
    try:
        ...
        if config.enable_phrase_cleanup:
            ...
            logger.info(
                f"✓ Phrase cleaner initialized (model: {config.cleanup_whisper_model})"
            )
        ...
        try:
                ...
                for future in as_completed(futures):
                    ...
                    if metadata.cleanup_status:
                        if metadata.cleanup_status == 'cleaned':
                            ...
                            logger.warning(
                                f"⚠️  Chunk {metadata.chunk_id}: Cleanup error, "
                    ...
                    if metadata.status.startswith("complete") and len(enhanced_audio) > 0:
                        ...
                        logger.info(
                            f"✓ Saved enhanced chunk {metadata.chunk_id}: {enhanced_path}"
                        )
            ...
            logger.info("=" * 60)
            logger.info(f"✓ Enhancement complete: {successful} successful, {failed} failed")
            logger.info(f"✓ Total processing time: {total_duration:.2f}s")
            if config.enable_phrase_cleanup:
                logger.info(f"🧹 Phrase cleanup: {phrases_cleaned_total} phrases removed from {chunks_with_phrases} chunks")
                if cleanup_errors > 0:
                    logger.warning(f"⚠️  Cleanup errors: {cleanup_errors} chunks had cleanup failures")
            logger.info("=" * 60)
